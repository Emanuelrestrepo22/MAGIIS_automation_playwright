...
type Phase2Status = 'active-canonical' | 'active-card-existing' | 'deprecated-redundant' | 'collapsed-alias';
...
const PROTECTED_PATTERNS: RegExp[] = [/E2E-FLOW/, /E2EFlow/, /E2E_/, /@e2e\b/];
...
async function applyChanges(absPath: string, ExcelJS: ExcelJsNs, byId: Map<string, NormalizedCase>): Promise<void> {
	const wb = new ExcelJS.Workbook();
	await wb.xlsx.readFile(absPath);
			...
			for (const p of plansForSheet) {
				...
				const extra = p.aliasCardExisting
					...
					: p.canonicalRef
						...
						: '';
				console.log(`    ${prefix} ${p.testCaseId}: [${p.action}] "${truncate(p.newTitle, 80)}"${extra}`);
			}
		} else {
			// Resumen por acción
...
async function main(): Promise<void> {
	...
	const targets = cli.onlyFile ? [cli.onlyFile] : DEFAULT_TARGETS;
	...
	for (const rep of reports) {
		...
		console.log(`  ${basename(rep.relative)}: matched=${matched}, orphans=${orphans}, missing=${missing}`);
	}
