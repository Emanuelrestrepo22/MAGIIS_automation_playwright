...
export async function readTestCases(filePath: string, sheetName?: string): Promise<TestCase[]> {
	...
	console.log(`✓ Leídos ${testCases.length} test cases desde "${targetSheet}" en ${path.basename(filePath)}`);
...
export function testCaseToPlaywrightBlock(tc: TestCase): string {
	const stepsComments = tc.steps.map((step, i) => `    // ${i + 1}. ${step}`).join('\n');

	const preconditionsComments = tc.preconditions.map(p => `  //   - ${p}`).join('\n');

	const testDataComment =
		Object.entries(tc.testData).length > 0
			? '  // Datos de prueba:\n' +
