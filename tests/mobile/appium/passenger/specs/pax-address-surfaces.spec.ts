...
test.describe('[MG-116] Guards de los campos de dirección — App PAX', () => {
	...
	test(
		'[TM-730][CONTROL] Editar viaje programado consulta el endpoint propio y no a Google',
		{ annotation: [{ type: 'tms', description: 'TM-730' }], tag: ['@regression', '@mg116'] },
		async () => {
			...
			try {
				...
				test.skip(
					!opened || !/travel-edit/i.test(url),
					'Sin viaje programado alcanzable en este entorno: guard no ejercido.'
				);
	...
	test(
		'[TM-729] Perfil › Mis Direcciones consulta el endpoint propio y no a Google',
		{ annotation: [{ type: 'tms', description: 'TM-729' }], tag: ['@regression', '@mg116'] },
		async () => {
			const { driver, webview } = await newSession();
			try {
	...
	test(
		'[TM-727] Las direcciones locales de cache se rankean por encima de aeropuertos lejanos',
		{ annotation: [{ type: 'tms', description: 'TM-727' }], tag: ['@regression', '@mg116'] },
		async () => {
			const { driver, webview } = await newSession();
			try {
				...
				for (let intento = 0; intento < 4 && !enHome; intento++) {
					...
					const tapeoInicio = await tapNativeByCss(
						driver,
						webview,
						'ion-tab-button, a, button, ion-item',
						'inicio'
					);
	...
	test(
		'[TM-731] travel-info sobrevive a una recarga de la WebView',
		{ annotation: [{ type: 'tms', description: 'TM-731' }], tag: ['@regression', '@mg116'] },
		async () => {
			...
			try {
				...
				const dead =
					!after || /no disponible|ERR_HTTP_RESPONSE_CODE_FAILURE/i.test(`${after.title} ${after.text}`);
				expect(
					dead,
