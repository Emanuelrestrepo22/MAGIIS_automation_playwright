...
export class PassengerTripStatusScreen extends AppiumSessionBase {
	...
	private async collectSnapshot(): Promise<StatusSnapshot | null> {
		try {
			...
			return await driver.execute(() => {
				...
				const visibleElements = Array.from(
					document.querySelectorAll(
						'button, [role="button"], ion-button, ion-item, ion-label, ion-tab-button, ion-col, span, div, a, p, h1, h2, h3'
					)
				)
					.filter(isVisible)
					.map(element => {
			...
			console.warn(
				'[PassengerTripStatusScreen] collectSnapshot failed:',
		...
		while (Date.now() < deadline) {
			...
			if (snapshot) {
				const haystack = this.normalizeText(
					[snapshot.url, ...snapshot.texts, ...snapshot.buttons, ...snapshot.matches].join(' ')
				);
	...
	async waitForTripCompleted(timeout = 120_000): Promise<void> {
		const ready = await this.waitForStatusKeywords(
			['completado', 'finalizado', 'cobro', 'pago', 'captured', 'resumen'],
			timeout
		);
		if (!ready) {
	...
	async getTripStatus(): Promise<string> {
		...
		const normalizedCandidates = [...snapshot.texts, ...snapshot.buttons, ...snapshot.matches];
