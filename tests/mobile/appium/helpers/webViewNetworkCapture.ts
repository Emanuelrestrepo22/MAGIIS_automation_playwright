...
export async function installWebViewNetworkCapture(driver: Browser): Promise<void> {
	...
	await driver.execute((storageKey: string, faultKey: string) => {
		 ...
		 */
		const shadowedProperties = ['readyState', 'status', 'statusText', 'responseText', 'response', 'responseURL', 'getAllResponseHeaders'];
		...
		const matchFaultRule = (kind: string, method: string, url: string): any => {
			...
			const hits: Record<string, number> = state.hits && typeof state.hits === 'object' ? state.hits : (state.hits = {});
			const budget: Record<string, number> = state.budget && typeof state.budget === 'object' ? state.budget : (state.budget = {});
			...
			for (const rule of rules) {
				...
				if (rule.matchMode === 'regex') {
					try {
						matched = new RegExp(rule.urlPattern, typeof rule.regexFlags === 'string' ? rule.regexFlags : '').test(target);
					} catch {
						matched = false;
					}
				} else {
			...
			win.fetch = (async (...args: any[]) => {
				...
				try {
					...
					if (faultRule) {
						...
						const faultDelay = typeof faultRule.delayMs === 'number' && faultRule.delayMs > 0 ? faultRule.delayMs : 0;
						const faultBase = {
							id,
							kind: 'fetch',
							method,
						...
						if (faultMode === 'timeout') {
							...
							const abortSignal: AbortSignal | null = init?.signal ?? (typeof Request !== 'undefined' && input instanceof Request ? input.signal : null);
							// Kept by reference so the parked entry can be corrected once it settles.
							const timeoutEntry: Record<string, unknown> = { ...faultBase, error: 'Injected fault: request hangs (timeout mode)' };
							...
							return new Promise((_resolve, reject) => {
								...
								if (abortSignal?.aborted) {
									settle(abortError(), 'Injected fault: signal was already aborted before the hang started');
									return;
								}
								...
								timer = win.setTimeout(() => settle(new TypeError('Injected fault: timeout elapsed'), 'Injected fault: timeout elapsed'), faultDelay > 0 ? faultDelay : 600000);
								...
								drain = registerPendingFault({
									...
									kind: 'fetch',
									url,
									cancel: () => settle(new TypeError('Injected fault: timeout cancelled on teardown'), 'Injected fault: timeout cancelled on teardown')
								});
							});
						}
						...
						try {
							return new Response(bodylessStatus ? null : faultBody, { status: faultStatus, headers: faultHeaders });
						} catch (responseError) {
		...
		if (!xhrProto.__magiisNetworkCaptureInstalled) {
			...
			xhrProto.open = function (method: string, url: string, ...rest: unknown[]) {
				...
				const parkedStatus = (instance as typeof instance & { __magiisParkedStatusFault?: ParkedStatusFault }).__magiisParkedStatusFault;
				if (parkedStatus) {
					try {
						parkedStatus.cancel();
					} catch {
				...
				delete (instance as typeof instance & { __magiisParkedStatusFault?: ParkedStatusFault }).__magiisParkedStatusFault;
			...
			xhrProto.send = function (body?: Document | XMLHttpRequestBodyInit | null) {
				...
				const previousStatusFault = (xhr as typeof xhr & { __magiisParkedStatusFault?: ParkedStatusFault }).__magiisParkedStatusFault;
				...
				delete (xhr as typeof xhr & { __magiisParkedStatusFault?: ParkedStatusFault }).__magiisParkedStatusFault;
				...
				if (context && faultRule) {
					...
					if (faultMode === 'timeout') {
						...
						(xhr as typeof xhr & { __magiisParkedFault?: ParkedFault }).__magiisParkedFault = { settle, discard };
						...
						if (ownTimeout > 0) {
							ownTimer = win.setTimeout(() => settle('timeout', `Injected fault: xhr.timeout (${ownTimeout} ms) elapsed while hanging`), ownTimeout);
						}
						...
						hangTimer = win.setTimeout(() => settle('timeout', 'Injected fault: timeout elapsed'), faultDelay > 0 ? faultDelay : 600000);
					...
					const faultStatus = isNetworkError ? 0 : typeof faultRule.status === 'number' ? faultRule.status : 503;
					const faultBody = isNetworkError || typeof faultRule.body !== 'string' ? '' : faultRule.body;
					const faultHeaderBlock = isNetworkError ? '' : `content-type: ${typeof faultRule.contentType === 'string' ? faultRule.contentType : 'application/json'}\r\n`;
					...
					(xhr as typeof xhr & { __magiisParkedStatusFault?: ParkedStatusFault }).__magiisParkedStatusFault = {
		...
		return true;
	}, NETWORK_CAPTURE_KEY, FAULT_INJECTION_KEY);
}
...
export async function installWebViewFaultInjection(driver: Browser, rules: WebViewFaultInjectionRule[]): Promise<void> {
	...
	const payload: SerializedFaultRule[] = rules.map((rule, index) => {
		...
		if (rule.mode === 'status' && typeof rule.status === 'number' && (!Number.isInteger(rule.status) || rule.status < 200 || rule.status > 599)) {
			throw new Error(`Fault-injection rule "${ruleId}" uses mode 'status' with status ${rule.status}, which is outside the 200-599 range the Response constructor accepts. Use mode: 'networkError' to simulate an absent response.`);
		}
		...
		if (rule.mode !== 'status' && rule.mode !== 'timeout' && rule.mode !== 'networkError') {
			throw new Error(`Fault-injection rule "${ruleId}" has mode '${String(rule.mode)}'. Valid modes are 'status', 'timeout' and 'networkError'.`);
		}
	...
	await driver.execute(
		(faultKey: string, ruleList: SerializedFaultRule[]) => {
			const win = window as any;
			const previous = win[faultKey] as { pending?: { cancel?: () => void }[]; hits?: Record<string, number> } | undefined;
			...
			const previousHits: Record<string, number> = previous && previous.hits && typeof previous.hits === 'object' ? previous.hits : {};
...
export async function readWebViewGoogleActivity(driver: Browser): Promise<WebViewGoogleActivity> {
	...
	const raw = (await driver
		...
		| { probeErrors: string[]; scriptTags: string[]; resourceEntries: { name: string; startTime: number }[]; sdkPresent: boolean }
		| { executeError: string };
...
export async function dumpWebViewNetworkCapture(driver: Browser, label: string): Promise<string> {
	...
	const summary = [
		...
		...capture.entries.flatMap((entry, index) => {
			...
			const injectedMarker = entry.injected ? ` [INJECTED mode=${entry.injectedMode ?? 'unknown'} rule=${entry.injectedRule ?? 'unknown'}]` : '';
			const lines = [`#${index + 1} [${entry.kind}]${injectedMarker} ${entry.method} ${entry.url}`, `  status: ${typeof entry.status === 'number' ? entry.status : '<n/a>'}`, `  ok: ${typeof entry.ok === 'boolean' ? String(entry.ok) : '<n/a>'}`, `  startedAt: ${entry.startedAt}`, `  endedAt: ${entry.endedAt ?? '<n/a>'}`, `  durationMs: ${typeof entry.durationMs === 'number' ? entry.durationMs : '<n/a>'}`, `  requestHeaders: ${formatHeaders(entry.requestHeaders)}`, `  requestBody: ${formatBody(entry.requestBody)}`, `  responseHeaders: ${formatHeaders(entry.responseHeaders)}`, `  responseBody: ${formatBody(entry.responseBody)}`];

			if (entry.injected) {
				lines.push(`  injected: true (rule: ${entry.injectedRule ?? '<unknown>'}, mode: ${entry.injectedMode ?? '<unknown>'})`);
			}
