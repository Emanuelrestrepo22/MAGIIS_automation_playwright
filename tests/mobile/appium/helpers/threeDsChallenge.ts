 ...
 */
export async function handleThreeDsPopup(
	driver: Browser,
	dumpState: (label: string) => Promise<string>,
	timeoutMs: number,
	dumpLabelPrefix: string
	...
	const isStripeCardEntryFrame = (frame: HTMLIFrameElement): boolean =>
		/elements-inner-card|cardnumber|cardexpiry|cardcvc|controller|metrics|hcaptcha/i.test(frameSignature(frame));
	const isStripeChallengeFrame = (frame: HTMLIFrameElement): boolean =>
		/three-ds-2-challenge|stripe-challenge-frame|3d_secure_2|acs|authenticate|verify|challenge/i.test(
			frameSignature(frame)
		) && !isStripeCardEntryFrame(frame);
	const switchFrameTarget = async (target: any): Promise<void> => {
		const browser = driver as Browser & {
			switchFrame?: (frame: any) => Promise<void>;
			switchToFrame?: (frame: any) => Promise<void>;
		};
	...
	while (Date.now() < deadline) {
		...
		for (const frame of iframeElements) {
			...
			if (
				...
				!/elements-inner-card|cardnumber|cardexpiry|cardcvc|controller|metrics|hcaptcha/i.test(signature)
			) {
		...
		const modalResult = (await driver.execute((texts: string[]) => {
			...
			const visible = overlays.filter(element => {
				...
				const hasChallengeText =
					/complete(?: authentication)?|authenticate|confirm|submit|approve|authoriz|3d\s*secure|challenge/i.test(
						text
					);
			...
			for (const overlay of visible) {
				const buttons = Array.from(
					overlay.querySelectorAll('button, [role="button"], a, input[type="submit"], input[type="button"]')
				) as HTMLElement[];
				for (const text of texts) {
					const target = normalize(text);
					const button = buttons.find(item => {
