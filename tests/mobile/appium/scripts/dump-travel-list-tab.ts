...
async function run() {
	...
	const dump = await driver
		.execute<string, []>(() => {
			...
			document
				.querySelectorAll(
					'ion-fab,ion-fab-button,[class*="fab"],[class*="calle"],[class*="street"],[class*="new-trip"],[class*="nueva"]'
				)
				.forEach(el => {
					const t = ((el as HTMLElement).innerText ?? '').trim().slice(0, 100);
					const cls = (el.className ?? '').toString().slice(0, 80);
			...
			document.querySelectorAll('ion-page:not(.ion-page-hidden)').forEach(el => {
				lines.push(
					`[ACTIVE PAGE] ${el.tagName} id="${el.id}" class="${(el.className ?? '').toString().slice(0, 80)}"`
				);
			});
