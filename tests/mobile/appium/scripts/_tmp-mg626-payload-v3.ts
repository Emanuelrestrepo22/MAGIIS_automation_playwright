...
async function readMode(driver: AppiumDriver): Promise<string> {
	...
	return driver
		.execute(() => {
			...
			const toggle = document.querySelector('app-home div.header-menu ion-toggle') as
				| (HTMLElement & { checked?: boolean })
				| null;
			const bodyHit = /Compa[nñ]ía/i.test(document.body?.innerText ?? '')
				? 'body-menciona-Compañía'
				: 'body-sin-Compañía';
...
function installNet(driver: AppiumDriver): Promise<unknown> {
	return driver.execute((k: string) => {
			...
			const w: any = async (...a: any[]) => {
				...
				try {
					...
					if (typeof i === 'string') url = i;
						...
						reqHeaders = hdrs(i.headers);
						reqBody = await bodyStr(i);
					}
		...
		if (!proto.__mg626X) {
			const oO = proto.open;
			const oS = proto.send;
			const oH = proto.setRequestHeader;
			proto.open = function (m: string, u: string, ...r: any[]) {
				(this as any).__x = {
					method: m || 'GET',
					url: String(u ?? ''),
			...
			proto.setRequestHeader = function (n: string, v: string) {
				...
				if (x) x.reqHeaders[n] = v;
				return oH.call(this, n, v);
			};
			proto.send = function (b?: any) {
				...
				const fin = (): void => {
					...
					entries.push({
						...
						at: x.at,
						status: s.status,
						reqHeaders: x.reqHeaders,
						reqBody: cut(x.reqBody ?? ''),
						resBody: cut(String(s.responseText ?? ''))
					});
				};
				s.addEventListener('loadend', fin);
				s.addEventListener('error', fin);
				s.addEventListener('abort', fin);
...
async function dumpComponentSource(driver: AppiumDriver): Promise<string> {
	...
	return driver
		.execute(() => {
			...
			try {
				parts.push(`### cardService.addCard\n${String(cmp.cardService?.addCard)}`);
			} catch {
				parts.push('### cardService.addCard\n<unreadable>');
			}
			try {
				parts.push(`### cardService keys\n${Object.keys(cmp.cardService ?? {}).join(', ')}`);
			} catch {
				/* noop */
			}
			return parts.join('\n\n');
...
async function run(): Promise<void> {
	...
	try {
		...
		log(
			`GUARDAR habilitado=${await wallet.isSaveEnabled()} · formValid=${JSON.stringify(await wallet.readFormValidity())}`
		);
		...
		for (let i = 0; i < 12; i++) {
			...
			if (entries.some(e => String(e.method).toUpperCase() === 'POST' && /cards/i.test(String(e.url)))) {
				log(`POST /cards capturado en muestreo ${i + 1}`);
				break;
			}
		}
		...
		try {
			...
			await wallet.openWallet();
			log(
				`wallet PERSONAL: hasCard(${LAST4})=${await wallet.hasCard(LAST4, 8_000).catch(() => false)} · count=${await wallet.countCards().catch(() => -1)}`
			);
			await snap(driver, 'v3-06-wallet-personal');
			const after = await netSnapshot(driver);
			const allCards = after
				.filter(e => /allCards/i.test(String(e.url)))
		...
		if (LEAVE_CLEAN) {
			...
			await wallet
				.deleteCard(LAST4)
				.catch((e: unknown) => log(`cleanup err: ${e instanceof Error ? e.message : String(e)}`));
