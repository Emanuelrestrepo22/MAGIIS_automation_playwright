	...
	try {
		...
		try {
			...
			await (driver as unknown as { saveScreenshot: (p: string) => Promise<unknown> }).saveScreenshot(
				'evidence/ebiz/newtrip-vehicle-v2517.png'
			);
			log('screenshot ok');
		} catch {
			/* noop */
		...
		const dump = await driver.execute(() => {
			...
			const pages = Array.from(document.querySelectorAll('ion-router-outlet > *, ion-modal > *, ion-app > *'))
				.map(e => `${e.tagName.toLowerCase()}${(e as HTMLElement).offsetParent === null ? '(h)' : ''}`)
				.filter((v, i, a) => a.indexOf(v) === i)
				.slice(0, 15);
			const btns = (
				Array.from(
					document.querySelectorAll('button, ion-button, .btn, [role="button"], ion-item')
				) as HTMLElement[]
			)
				.filter(isVisible)
				.map(e => ({
...
run().catch((e: unknown) => {
	console.error(`[veh-dump] ${e instanceof Error ? e.message : String(e)}`);
	process.exit(1);
});
