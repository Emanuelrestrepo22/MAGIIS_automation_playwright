...
async function run(): Promise<void> {
	...
	const chSelect = chScalar
		.map(c =>
			['DATE', 'TIMESTAMP(6)', 'TIMESTAMP'].includes(c.type)
				? `${DT(c.col)} AS "${c.col}"`
				: `${c.col} AS "${c.col}"`
		)
		.join(', ');
	if (chSelect) {
		const chRows = await db
			...
			>(`SELECT ${chSelect} FROM MAGIIS.CARD_HOLDS ORDER BY ID DESC FETCH FIRST 8 ROWS ONLY`)
			.catch((e: unknown) => {
				log(`err CARD_HOLDS: ${e instanceof Error ? e.message : String(e)}`);
				return [];
			});
	...
	const tx = await db
		...
		.catch((e: unknown) => {
			log(`err MGW_TX: ${e instanceof Error ? e.message : String(e)}`);
			return [];
		});
...
run().catch((e: unknown) => {
	console.error(`[mgw-db] ${e instanceof Error ? e.message : String(e)}`);
	process.exit(1);
});
