	...
	const dump = await driver
		.execute<string, []>(() => {
			...
			const activePage = document.querySelector(
				'page-home:not(.ion-page-hidden), app-navigator:not(.ion-page-hidden)'
			);
			const scope = activePage ?? document;
			(
				scope.querySelectorAll(
					'button,[role="button"],ion-button,ion-fab-button,ion-item,a'
			...
			document.querySelectorAll('ion-page:not(.ion-page-hidden), page-home:not(.ion-page-hidden)').forEach(el => {
				lines.push(
					`[ACTIVE] ${el.tagName} id="${el.id}" class="${(el.className ?? '').toString().slice(0, 80)}"`
				);
			});
