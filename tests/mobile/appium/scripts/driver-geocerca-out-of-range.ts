...
async function switchWv(d: Driver, timeout = 12_000): Promise<boolean> {
	...
	while (Date.now() < deadline) {
		...
		if (wv) {
			await d.switchContext(wv).catch(() => {});
			return true;
		}
		await d.pause(300);
	}
		...
		.execute<Record<string, unknown>, []>(() => {
			...
			for (const sel of [
				...
				document.querySelectorAll(sel).forEach(el => {
					if (!vis(el)) return;
					const buttons = Array.from(el.querySelectorAll('button, .alert-button'))
						.filter(vis)
	...
	try {
		...
		let assignedSeen = '';
		{
			...
			while (Date.now() < deadline && !confirmReached) {
				...
				const probe = await driver
					.execute<{ confirm: boolean; assigned: string; homeAssignedText: string }, []>(() => {
						const norm = (v: unknown): string =>
							String(v ?? '')
								.replace(/\s+/g, ' ')
								.trim();
						...
						const cards = Array.from(
							document.querySelectorAll(
								'page-home:not(.ion-page-hidden) [class*="trip"], page-home:not(.ion-page-hidden) [class*="travel"], page-home:not(.ion-page-hidden) ion-card, page-home:not(.ion-page-hidden) [class*="assigned"], page-home:not(.ion-page-hidden) [class*="asignad"]'
							)
						);
						for (const c of cards) {
							if (!vis(c)) continue;
							const t = norm((c as HTMLElement).innerText);
							if (/\d{2,}\s*-\s*[A-Z]/.test(t) || /\$\s*\d/.test(t)) {
								assigned = t.slice(0, 120);
								(c as HTMLElement).click();
								break;
							}
						}
						const homeAssignedText = norm(
							document.querySelector('page-home:not(.ion-page-hidden)')?.textContent ?? ''
						).slice(0, 400);
						return { confirm: confirmDom, assigned, homeAssignedText };
					})
					.catch(() => ({ confirm: false, assigned: '', homeAssignedText: '' }));
				if (probe.assigned) {
					assignedSeen = probe.assigned;
				...
				if (/TravelConfirmPage/i.test(await url(driver))) {
					confirmReached = true;
					break;
				}
				await driver.pause(1200);
		...
		if (!confirmReached) {
			add(
				'WAIT-CONFIRM',
				'FAIL',
				`no llegó al viaje asignado en ${CONFIRM_TIMEOUT}ms. assignedCardVista="${assignedSeen}" url=${await url(driver)}. (Con pickup lejano el backend no empuja el viaje al device driver — ver GAP.)`
			);
			return;
		}
		// capturar travelId
		...
		await driver
			.execute<boolean, []>(() => {
				const active =
					document.querySelector('app-page-travel-to-start:not(.ion-page-hidden)') ??
					document.querySelector('app-page-travel-to-start');
				const b =
					(active?.querySelector('ion-footer button') as HTMLElement | null) ??
					(document.querySelector('button.btn.primary.trip-pax-start') as HTMLElement | null);
		...
		if (alreadyInProgress) {
			add(
				'GEOCERCA',
				'PARTIAL',
				'llegó a TravelInProgressPage sin pedir código (in-range). El pickup NO estaba fuera de rango.'
			);
		} else {
			// ¿hay botón "Ingresar código"?
			const hasCodeBtn = await driver
				.execute<boolean, []>(() => {
					const btns = Array.from(
			...
			if (hasCodeBtn) {
				...
				await driver
					.execute<boolean, []>(() => {
						const btns = Array.from(
							document.querySelectorAll(
								'app-confirm-modal button.btn.primary, ion-modal button.btn.primary, app-confirm-modal button, ion-modal button'
							)
						) as HTMLElement[];
						const b = btns.find(
							x => x.offsetParent !== null && /ingresar c[oó]digo/i.test(x.textContent ?? '')
						);
						if (b) {
							b.click();
							return true;
				...
				if (code.length < 4) {
					...
					const confirmed = await driver
						.execute<boolean, []>(() => {
							const btns = Array.from(
								document.querySelectorAll(
									'app-code-confirmation-modal button.btn.primary, ion-modal button.btn.primary, app-code-confirmation-modal button, ion-modal button'
								)
							) as HTMLElement[];
					...
					add(
						'GEOCERCA-CODE',
						inProgress ? 'OK' : 'FAIL',
						`code=${code} confirmar=${confirmed} → TravelInProgressPage=${inProgress} (url=${await url(driver)})`
					);
				}
			} else {
				add(
					'GEOCERCA-MODAL',
					'FAIL',
					`no apareció "Ingresar código" tras Empezar Viaje. overlays=${JSON.stringify(ov1)}`
				);
			}
		}
					...
					await driver
						.execute<boolean, []>(() => {
							const b = document.querySelector(
								'app-page-signer ion-footer button.btn.primary, app-confirm-modal button.btn.primary, app-alert-modal button'
							) as HTMLElement | null;
							if (b && b.offsetParent !== null) {
								b.click();
								return true;
		...
		add(
			'CLEANUP',
			/\/navigator\/home/i.test(await url(driver)) ? 'OK' : 'PARTIAL',
			`url final=${await url(driver)}`
		);
	} catch (e) {
		...
		const ts = new Date().toISOString().replace(/[:.]/g, '-');
		writeFileSync(
			join('evidence/reports', `geocerca-out-of-range-${ts}.json`),
...
run().catch(e => {
	console.error('[geocerca] fatal:', e);
	process.exit(1);
});
