/**
 * DOM dump for the Driver App home screen.
 *
 * Expected usage:
 ...
 *   npx ts-node --esm tests/mobile/appium/scripts/dump-driver-home.ts
 */
...
type DriverHomeWebDump = {
	...
	availableLabelText: string;
	tripCardMatches: string[];
};
...
async function switchToWebView(driver: WebdriverIO.Browser): Promise<string | null> {
	...
	console.log(`[dump-driver-home] Contexts: ${contexts.join(', ')}`);
	const webview = contexts.find(context => context.startsWith('WEBVIEW'));
	if (!webview) {
		console.warn('[dump-driver-home] No WEBVIEW context found');
		return null;
	}

	await driver.switchContext(webview);
	console.log(`[dump-driver-home] Switched to ${webview}`);
	return webview;
}

async function switchToNative(driver: WebdriverIO.Browser): Promise<void> {
	await driver.switchContext('NATIVE_APP');
	console.log('[dump-driver-home] Switched to NATIVE_APP');
}

async function collectWebDump(driver: WebdriverIO.Browser): Promise<DriverHomeWebDump | null> {
	...
	const dump = await driver.execute(() => {
		...
		const availableLabelText = normalize(document.querySelector('.available-label')?.textContent || '');

		const tripCardMatches = elements
			.filter(item => /trip|viaje|request|solicitud|active|activo/i.test(`${item.id} ${item.accessibilityId} ${item.text} ${item.className} ${item.role}`))
...
function formatWebDump(dump: DriverHomeWebDump | null): string {
	if (!dump) {
		return [
			'=== WEBVIEW DUMP ===',
	...
	return [
		'=== WEBVIEW DUMP ===',
		`URL: ${dump.url}`,
		`Availability found: ${dump.availabilityFound}`,
		`Availability text: ${dump.availabilityText || '(none)'}`,
		`Available label: ${dump.availableLabelText || '(none)'}`,
		`Buttons: ${dump.buttonTexts.length ? dump.buttonTexts.join(' | ') : '(none)'}`,
		'',
		'--- Trip Card / Active Trip Matches ---',
		dump.tripCardMatches.length ? dump.tripCardMatches.join('\n') : '(none)',
		'',
...
async function run(): Promise<void> {
	...
	console.log(`[dump-driver-home] Connecting to ${APPIUM_URL} | device=${DEVICE_NAME} | udid=${UDID}`);
	...
	try {
		const webDump = await collectWebDump(driver);
		const nativeDump = await collectNativeDump(driver);
		await ensureOutputDir();

		const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
		const outputPath = path.join(OUTPUT_DIR, `driver-home-${timestamp}.txt`);
		const contents = [
		...
		console.log(`\n${contents}\n`);
		console.log(`[dump-driver-home] Saved to ${outputPath}`);
	} finally {
...
run().catch(error => {
	console.error('[dump-driver-home] Error:', error instanceof Error ? error.message : error);
	process.exit(1);
});
