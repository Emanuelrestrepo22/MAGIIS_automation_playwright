...
async function run(): Promise<void> {
	...
	try {
		...
		log(`URL: ${url0}`);
		if (!/TravelResumePage/i.test(url0)) { log('⚠ No estamos en TravelResumePage. Abortando.'); return; }
		...
		const footerState = async (): Promise<{ text: string; disabled: boolean }> => driver.execute<{ text: string; disabled: boolean }, []>(() => {
		...
		const payCount = await driver.execute<number, []>(() => {
			const resume = document.querySelector('app-travel-resume:not(.ion-page-hidden)') || document.querySelector('app-travel-resume');
			return Array.from(resume?.querySelectorAll('.travel-payment button.payment') || []).filter((b) => (b as HTMLElement).offsetParent !== null).length;
		}).catch(() => 0);
		...
		for (let i = 0; i < Math.max(payCount, 2) && !/ingresar tarjeta/i.test(fs.text); i++) {
			await driver.execute<boolean, [number]>((idx) => {
				const resume = document.querySelector('app-travel-resume:not(.ion-page-hidden)') || document.querySelector('app-travel-resume');
				const pays = Array.from(resume?.querySelectorAll('.travel-payment button.payment') || []).filter((b) => (b as HTMLElement).offsetParent !== null) as HTMLElement[];
		...
		if (!/ingresar tarjeta/i.test(fs.text)) { log('⚠ No se logró footer="Ingresar tarjeta" (CREDIT_CARD). Abortando (NO cierro con otro método).'); return; }
		if (fs.disabled) { log('⚠ "Ingresar tarjeta" DISABLED (totalCostFinal=0 en viaje stale). Abortando.'); return; }
		...
		await switchWv();
		const modalInfo = await driver.execute<Record<string, unknown>, []>(() => {
			const probe = (sel: string) => {
				const h = document.querySelector(sel) as (HTMLElement & { shadowRoot?: ShadowRoot }) | null;
				if (!h) return { found: false };
				const inner = (h.shadowRoot ? h.shadowRoot.querySelector('input') : h.querySelector('input')) as HTMLInputElement | null;
				return { found: true, hasShadow: !!h.shadowRoot, hasInner: !!inner, readOnly: inner?.readOnly ?? null };
			};
			return {
				url: window.location.href,
				cardNumber: probe('#cardNumber'), cardExpirationDate: probe('#cardExpirationDate'),
				securityCode: probe('#securityCode'), cardholderName: probe('#cardholderName'),
				chargeSpans: Array.from(document.querySelectorAll('.header.end span, ion-header span')).map((s) => ({ cls: (s as HTMLElement).className, text: String((s as HTMLElement).innerText ?? '').trim() })),
			};
		}).catch((e) => ({ error: String(e) }));
		log(`MODAL info: ${JSON.stringify(modalInfo)}`);
		if (!ready) { log('⚠ Modal no presente. Abortando.'); return; }
		...
		await driver.pause(1500);
		await switchWv();
		const afterFill = await driver.execute<Record<string, unknown>, []>(() => {
			const val = (sel: string) => {
				const h = document.querySelector(sel) as (HTMLElement & { shadowRoot?: ShadowRoot }) | null;
				const inner = h && (h.shadowRoot ? h.shadowRoot.querySelector('input') : h.querySelector('input')) as HTMLInputElement | null;
				return inner ? inner.value : null;
			};
			const chargeValid = Array.from(document.querySelectorAll('.header.end span.title')).some((s) => /cobrar/i.test((s as HTMLElement).innerText || ''));
			const chargeInvalid = !!document.querySelector('span.invalid-charge');
			return { cardNumber: val('#cardNumber'), exp: val('#cardExpirationDate'), cvc: val('#securityCode'), holder: val('#cardholderName'), chargeValidVisible: chargeValid, chargeInvalidVisible: chargeInvalid };
		}).catch((e) => ({ error: String(e) }));
...
run().catch((e) => { console.error('[charge] fatal:', e); process.exit(1); });
