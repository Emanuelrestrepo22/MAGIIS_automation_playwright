	...
	try {
		...
		const info = await driver
			.execute<Record<string, unknown>, []>(() => {
				...
				document
					.querySelectorAll(
						'[class*="assigned"], [class*="asignado"], [class*="trip-card"], [class*="travel-card"], [class*="available"], .home-section, ion-card'
					)
					.forEach(el => {
						if (vis(el)) {
							const t = norm((el as HTMLElement).innerText);
							if (t) sections.push(t.slice(0, 160));
						}
					});
				// textos clave del home
				const bodyText = norm(
					document.querySelector('page-home:not(.ion-page-hidden)')?.textContent ?? document.body.textContent
				).slice(0, 800);
				return {
					url: window.location.href,
					availability: norm(
						(document.querySelector('#availability') as HTMLElement | null)?.textContent ?? ''
					),
		...
		try {
			...
			src = (await driver.getPageSource()).slice(0, 4000);
			if (wv) await driver.switchContext(wv);
		} catch {
			/* noop */
		}
		...
		writeFileSync(
			join('evidence/dom-dump', `assigned-trip-state-${ts}.txt`),
			JSON.stringify({ info, nativeSrcHead: src }, null, 2),
			'utf-8'
		);
...
run().catch(e => {
	console.error('[assigned-state] fatal:', e);
	process.exit(1);
});
