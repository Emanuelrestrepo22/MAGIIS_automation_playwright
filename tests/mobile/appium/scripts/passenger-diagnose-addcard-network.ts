...
async function run(): Promise<void> {
	const card = STRIPE_TEST_CARDS[(process.env.PASSENGER_CARD_KEY ?? 'visa_success') as keyof typeof STRIPE_TEST_CARDS];
	...
	try {
		...
		const onCards = async (): Promise<boolean> => driver.execute(() => window.location.href.includes('/cards')).catch(() => false);
		for (let i = 0; i < 4 && !(await onCards()); i++) {
			await driver.execute(() => { (document.querySelector('#app-tab-bar ion-menu-toggle, ion-menu-toggle, ion-menu-button') as HTMLElement | null)?.click(); }).catch(() => {});
			await driver.pause(1_200);
			await driver.execute(() => {
				const b = (Array.from(document.querySelectorAll('ion-item, ion-label, a, button, span, div')) as HTMLElement[]).find(e => /^\s*billetera\s*$/i.test(e.textContent ?? '') && e.offsetParent !== null);
				b?.click();
			}).catch(() => {});
		...
		const cardInputPresent = async (): Promise<boolean> => driver.execute(() => !!document.querySelector('ion-input[formcontrolname="cardNumber"] input')).catch(() => false);
		let ready = false;
		for (let a = 0; a < 3 && !ready; a++) {
			await driver.execute(() => {
				const t = (Array.from(document.querySelectorAll('.btn.primary, ion-button')) as HTMLElement[]).find(b => /agregar/i.test(b.textContent ?? '') && b.offsetParent !== null);
				t?.click();
			}).catch(() => {});
			for (let w = 0; w < 30 && !ready; w++) { await driver.pause(500); ready = await cardInputPresent(); }
		}
		if (!ready) throw new Error('form #cardNumber no montó');
		log('Form montado.');

		const typeField = async (fcn: string, value: string): Promise<void> => {
			await driver.execute((name: string) => {
				const n = document.querySelector(`ion-input[formcontrolname="${name}"] input`) as HTMLInputElement | null;
				n?.focus(); n?.click();
			}, fcn).catch(() => {});
		...
		for (let i = 0; i < 24 && submitted !== 'clicked'; i++) {
			submitted = await driver.execute(() => {
				const modal = (Array.from(document.querySelectorAll('app-credit-card-payment-data')) as HTMLElement[]).find(m => m.offsetParent !== null) ?? document.body;
				const b = (Array.from(modal.querySelectorAll('button.btn.primary, ion-button.btn.primary, button[type="submit"], ion-button')) as HTMLElement[])
					.find(x => x.offsetParent !== null && !((x as HTMLButtonElement).disabled || x.getAttribute('disabled') !== null) && /agregar|guardar|confirmar|continuar|a[ñn]adir/i.test(x.textContent ?? ''));
...
run().catch((e: unknown) => { console.error(`[diag-addcard] ${e instanceof Error ? e.message : String(e)}`); process.exit(1); });
