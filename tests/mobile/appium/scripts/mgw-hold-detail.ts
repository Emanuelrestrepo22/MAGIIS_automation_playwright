...
async function run(): Promise<void> {
	...
	const holds = await db
		.query<Record<string, unknown>>(
		   ...
		   FROM MAGIIS.CARD_HOLDS WHERE TRAVEL_ID IN (${inList}) ORDER BY ID DESC`,
			binds
		)
		.catch((e: unknown) => {
			log(`err holds: ${e instanceof Error ? e.message : String(e)}`);
			return [];
		});
	log(`CARD_HOLDS:`);
	for (const r of holds) log(JSON.stringify(r));
	...
	const tx = await db
		.query<Record<string, unknown>>(
		   ...
		   FROM MAGIIS.MGW_TRANSACTIONS WHERE TRANSACTION_REF IN (${inList}) ORDER BY ID DESC`,
			binds
		)
		.catch((e: unknown) => {
			log(`err tx: ${e instanceof Error ? e.message : String(e)}`);
			return [];
		});
	log(`MGW_TRANSACTIONS:`);
	for (const r of tx) log(JSON.stringify(r));
}

run().catch((e: unknown) => {
	console.error(`[hold-detail] ${e instanceof Error ? e.message : String(e)}`);
	process.exit(1);
});
