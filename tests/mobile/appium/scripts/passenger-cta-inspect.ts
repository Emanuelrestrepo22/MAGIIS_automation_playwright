...
async function run(): Promise<void> {
	...
	try {
		...
		const info = await driver.execute(() => {
			...
			const leaf = all.find(e => norm(e.textContent).includes('seleccionar veh') && !Array.from(e.children).some(c => norm((c as HTMLElement).textContent).includes('seleccionar veh')));
			...
			for (let n = 0, cur: HTMLElement | null = leaf; n < 5 && cur; n++, cur = cur.parentElement) {
				...
				if (t === 'button' || t === 'ion-button' || cur.getAttribute('role') === 'button' || (cur.className || '').toString().includes('btn')) { clickable = cur; break; }
			}
			...
			return {
				found: true,
				leafTag: leaf.tagName.toLowerCase(), leafCls: (leaf.className || '').toString().slice(0, 80),
				clickTag: clickable.tagName.toLowerCase(), clickCls: (clickable.className || '').toString().slice(0, 100),
				attrs,
		...
		let last = '';
		for (let i = 0; i < 12; i++) {
			const state = await driver.execute(() => ({ url: location.href, page: (document.querySelector('ion-router-outlet > *:last-child, ion-modal') || {}).tagName || '', has: (document.body?.innerText || '').replace(/\s+/g, ' ').slice(0, 60) }));
			const s = JSON.stringify(state);
			if (s !== last) { log(`t+${i * 500}ms: ${s}`); last = s; }
...
run().catch((e: unknown) => { console.error(`[cta] ${e instanceof Error ? e.message : String(e)}`); process.exit(1); });
