...
async function run(): Promise<void> {
	...
	const dumpResume = async (): Promise<Record<string, unknown>> =>
		driver
			.execute<Record<string, unknown>, []>(() => {
				...
				const pays = Array.from(r?.querySelectorAll('.travel-payment button.payment') ?? []).map(b => ({
					...
					class: norm((b as HTMLElement).className).slice(0, 60),
					icon: b.querySelector('ion-icon')?.getAttribute('src') ?? '',
					vis: vis(b)
				}));
	...
	try {
		...
		if (!/TravelResumePage/i.test(u)) {
			log('No en TravelResumePage — nada que liberar.');
			return;
		}
		...
		for (let i = 0; i < Math.max(payCount, 3); i++) {
			...
			if (!/ingresar tarjeta/i.test(String(fs.footerText)) && fs.footerDisabled === false) {
				...
				// firma
				const hasSigner = await driver
					.execute<boolean, []>(() => !!document.querySelector('app-page-signer'))
					.catch(() => false);
				if (hasSigner) {
					log('  firma presente — dibujando');
					const rect = await driver
						.execute<{ x: number; y: number; w: number; h: number } | null, []>(() => {
						...
						.catch(() => null);
					if (rect && rect.w > 5) {
						const px = (f: number) => Math.round(rect.x + rect.w * f);
						const py = (f: number) => Math.round(rect.y + rect.h * f);
						await driver
							.performActions([
								{
									type: 'pointer',
									...
									actions: [
										{ type: 'pointerMove', duration: 0, x: px(0.2), y: py(0.5) },
										{ type: 'pointerDown', button: 0 },
										{ type: 'pointerMove', duration: 120, x: px(0.5), y: py(0.3) },
										{ type: 'pointerMove', duration: 120, x: px(0.8), y: py(0.6) },
					...
					await driver
						.execute<boolean, []>(() => {
							const b = document.querySelector(
								'app-page-signer ion-footer button.btn.primary'
							) as HTMLElement | null;
							if (b) {
								b.click();
								return true;
				...
				await driver
					.execute<boolean, []>(() => {
						const b = document.querySelector(
							'app-alert-modal button.btn-outlined-red, app-alert-modal button'
						) as HTMLElement | null;
...
run().catch(e => {
	console.error('[free-resume] fatal:', e);
	process.exit(1);
});
