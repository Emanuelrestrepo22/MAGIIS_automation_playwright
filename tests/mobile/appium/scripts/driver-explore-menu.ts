...
async function enumerate(driver: Driver, key: string): Promise<void> {
	...
	const dump = await driver
		.execute(() => {
			...
			const q =
				'button, [role="button"], ion-button, ion-item[button], ion-item[detail], ion-toggle, ion-segment-button, ion-input, ion-select, ion-back-button, ion-fab-button, ion-checkbox, ion-radio';
			const interactive = (Array.from(active.querySelectorAll(q)) as HTMLElement[]).filter(vis).map(el => ({
				...
				selector: selOf(el),
				icons: Array.from(el.querySelectorAll('ion-icon'))
					.map(ic => norm(ic.getAttribute('name') || ic.getAttribute('icon')))
					.filter(Boolean)
			}));
			const headings = Array.from(active.querySelectorAll('ion-title, h1, h2, .title, ion-label'))
				.map(h => norm(h.textContent))
				.filter(Boolean)
				.slice(0, 12);
			return {
				url: window.location.href,
				pageTag: active.tagName ? active.tagName.toLowerCase() : '',
				headings,
				interactive
			};
		})
	...
	const d = dump as {
		...
		pageTag: string;
		headings: string[];
		interactive: Array<{ kind: string; selector: string; text: string; icons: string[] }>;
		_err?: string;
	};
	log(
		`SCREEN "${key}" url=${d.url} page=${d.pageTag} headings=[${d.headings.join(' | ')}]${d._err ? ' ERR=' + d._err : ''}`
	);
	for (const it of d.interactive)
		log(
			`   • ${it.kind} sel=${it.selector} text="${it.text}" ${it.icons.length ? 'icons=' + it.icons.join(',') : ''}`
		);
}
...
async function clickMenuItem(driver: Driver, label: string): Promise<boolean> {
	const res = await driver
		.execute((lbl: string) => {
			...
			const items = Array.from(document.querySelectorAll('ion-item.menu-link-url')) as HTMLElement[];
			const m = items.find(
				i => i.offsetParent !== null && norm(i.innerText || i.textContent).includes(norm(lbl))
			);
			if (m) {
				m.click();
				return true;
...
async function run(): Promise<void> {
	...
	try {
		...
		for (const label of labels) {
			...
			if (!opened) {
				log(`could not open menu for ${label}`);
				continue;
			}
