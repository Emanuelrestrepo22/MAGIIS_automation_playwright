...
async function run() {
	...
	const tapped = await driver.execute<string, []>(() => {
		const btns = Array.from(
			document.querySelectorAll('button.driver-home.home-icon-base, button.driver-home')
		) as HTMLButtonElement[];
		const visible = btns.find(b => b.offsetParent !== null);
		if (visible) {
			visible.click();
			return (visible.className ?? '') + ' | ' + (visible.innerText ?? '').trim();
		}
		return '';
	});
		...
		.execute<string, []>(() => {
			...
			(
				document.querySelectorAll(
					'button,[role="button"],ion-button,input,select,ion-select,ion-input'
			...
			document
				.querySelectorAll(
					'ion-page:not(.ion-page-hidden),app-page-new-travel,app-travel-calle,[class*="calle"],[class*="street-trip"],[class*="new-travel"]'
				)
				.forEach(el => {
					lines.push(`[ACTIVE] ${el.tagName} class="${(el.className ?? '').toString().slice(0, 100)}"`);
				});
			(document.querySelectorAll('ion-title,ion-toolbar,h1,h2,ion-header') as NodeListOf<HTMLElement>).forEach(
				el => {
