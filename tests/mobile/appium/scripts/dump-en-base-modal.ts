...
async function run(): Promise<void> {
	...
	try {
		...
		const tapped = await driver
			.execute<boolean, []>(() => {
				const active =
					document.querySelector('page-home:not(.ion-page-hidden), .ion-page:not(.ion-page-hidden)') ??
					document;
		...
		const dump = await driver
			.execute<Record<string, unknown>, []>(() => {
				...
				const modals = Array.from(
					document.querySelectorAll('ion-modal, app-base-modal, app-nearby-bases, [class*="base"]')
				).filter(vis);
				...
				for (const m of modals) {
					...
					const items = Array.from(m.querySelectorAll('ion-item, ion-radio, .base-item, li, ion-row'))
						.filter(vis)
						.slice(0, 12)
						.map(it => ({
							tag: it.tagName.toLowerCase(),
							cls: norm((it as HTMLElement).className).slice(0, 50),
							text: norm((it as HTMLElement).innerText).slice(0, 60)
						}));
					const btns = Array.from(m.querySelectorAll('button, ion-button'))
						.filter(vis)
						.map(b => ({
							text: norm((b as HTMLElement).innerText),
							cls: norm((b as HTMLElement).className).slice(0, 50)
						}));
					const header = norm(
						m.querySelector('ion-header, ion-title, h1, h2, .modal-title')?.textContent ?? ''
					);
		...
		let dismissed = await driver
			.execute<boolean, []>(() => {
				const bd = document.querySelector('ion-modal ion-backdrop, ion-backdrop') as HTMLElement | null;
				if (bd && bd.offsetParent !== null) {
					bd.click();
					return true;
				}
				// botón cerrar/atrás del modal
				const closeBtn = document.querySelector(
					'ion-modal ion-buttons button, ion-modal .modal-close, ion-modal button[aria-label*="close" i]'
				) as HTMLElement | null;
				if (closeBtn) {
					closeBtn.click();
					return true;
