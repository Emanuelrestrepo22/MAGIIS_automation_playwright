...
async function run(): Promise<void> {
	...
	try {
		...
		log(`wallet abierto. tarjetas antes = ${countBefore}. ¿ya existe last4 ${LAST4}? ${await wallet.hasCard(LAST4, 3_000)}`);
		...
		const fillField = async (fcn: string, value: string): Promise<string> =>
			driver.execute((controlName: string, val: string) => {
				const host = document.querySelector(`ion-input[formcontrolname="${controlName}"], #${controlName}, input[formcontrolname="${controlName}"]`) as HTMLElement | null;
				...
				try { (host as HTMLElement & { value?: string }).value = val; } catch { /* ion-input value prop */ }
				...
				const readback = (host.querySelector('input.native-input, input') as HTMLInputElement | null)?.value ?? '';
				return `ok(readback="${readback}")`;
			}, fcn, value).catch((e: unknown) => `err:${e instanceof Error ? e.message : String(e)}`) as Promise<string>;
		...
		const zipNow = await driver.execute(() => {
			const host = document.querySelector('ion-input[formcontrolname="zipCode"], #zipCode') as HTMLElement | null;
		...
		const formInfo = await driver.execute(() => {
			...
			const findGroup = (o: Record<string, unknown>): { name: string; group: any } | null => {
				for (const k of Object.keys(o)) {
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					const v = (o as any)[k];
					if (v && typeof v === 'object' && v.controls && typeof v.controls === 'object') return { name: k, group: v };
				}
				return null;
			};
		...
		const saveState = await driver.execute(() => {
		...
		while (Date.now() < deadline) {
			const state = await driver.execute(() => {
				...
				const errorHit = /(rechaz|declined|inválid|invalid|error|no se pudo|failed)/i.test(bodyText.slice(0, 4000));
				return { modalGone, errorHit };
			}).catch(() => ({ modalGone: false, errorHit: false }));
			if (state.modalGone) { outcome = 'modal-closed'; break; }
			if (state.errorHit) { outcome = 'error-text-visible'; break; }
		...
		log(appears ? '✅ GO — tarjeta eBiz vinculada y visible en el wallet' : '❌ NO-GO — la tarjeta eBiz no quedó visible en el wallet');
	} finally {
		await harness.endSession();
	}
}

run().catch((e: unknown) => { console.error(`[ebiz-addcard] ${e instanceof Error ? e.message : String(e)}`); process.exit(1); });
