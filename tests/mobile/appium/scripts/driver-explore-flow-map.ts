...
type Interactive = {
  kind: string; id: string; class: string; text: string; aria: string; role: string;
  selector: string; inMenu: boolean; disabled: boolean;
  tab?: string; href?: string; icons?: string[]; labels?: string[];
};
type ScreenDump = {
  key: string; url: string; pageTag: string; componentTags: string[]; title: string;
  interactive: Interactive[]; note?: string;
};
...
async function persist(): Promise<void> {
  ...
  await writeFile(OUT_JSON, JSON.stringify({
    timestamp: new Date().toISOString(), device: UDID, appPackage: APP_PACKAGE,
    screens, transitions,
  }, null, 2), 'utf-8');
}
...
async function enumerate(driver: Driver, key: string, note?: string): Promise<ScreenDump> {
  ...
  const dump = await driver.execute(() => {
    ...
    const vis = (el: Element): boolean => {
      const h = el as HTMLElement; const r = h.getBoundingClientRect(); const s = getComputedStyle(h);
      return s.display !== 'none' && s.visibility !== 'hidden' && r.width > 0 && r.height > 0;
    };
    const esc = (s: string): string => (window.CSS && CSS.escape ? CSS.escape(s) : s.replace(/([^\w-])/g, '\\$1'));
    ...
    const componentTags = Array.from(new Set(
      Array.from(document.querySelectorAll('[class*="ion-page"]:not(.ion-page-hidden), app-, page-'))
        .concat(Array.from(document.querySelectorAll('*')).filter((e) => /^(app|page)-/.test(e.tagName.toLowerCase()) && vis(e)))
        .map((e) => e.tagName.toLowerCase())
        .filter((t) => /^(app|page)-/.test(t)),
    )).slice(0, 25);
    const menu = document.querySelector('ion-menu.show-menu, ion-menu.menu-pane-visible, ion-menu:not([style*="display: none"])');
    const q = 'button, [role="button"], ion-button, ion-tab-button, ion-menu-button, ion-menu-toggle, a[href], ion-item[button], ion-item[detail], ion-fab-button, ion-toggle, ion-segment-button, ion-back-button, [id="availability"]';
    ...
    const interactive = (Array.from(document.querySelectorAll(q)) as HTMLElement[])
      ...
      .map((el) => {
        ...
        const labels = Array.from(el.querySelectorAll('ion-label, .label, span'))
          .map((l) => norm(l.textContent)).filter(Boolean).slice(0, 3);
        const item = {
          kind: el.tagName.toLowerCase(),
          id: norm(el.id),
          class: norm(typeof el.className === 'string' ? el.className : ''),
          text: norm(el.innerText || el.textContent).slice(0, 60),
          aria: norm(el.getAttribute('aria-label') || el.getAttribute('content-desc') || el.getAttribute('title')),
          role: norm(el.getAttribute('role')),
          selector: selOf(el),
          inMenu: !!(menu && menu.contains(el)),
          disabled: (el as HTMLButtonElement).disabled === true || el.getAttribute('disabled') !== null || el.getAttribute('aria-disabled') === 'true',
    ...
    return {
      ...
      pageTag,
      componentTags,
      title: norm(active.querySelector('ion-title')?.textContent || document.querySelector('.ion-page:not(.ion-page-hidden) ion-title')?.textContent || document.title),
      interactive,
    };
  }).catch((e: Error) => ({ url: '', pageTag: '', componentTags: [], title: '', interactive: [], _err: e.message })) as Omit<ScreenDump, 'key' | 'note'> & { _err?: string };

  const screen: ScreenDump = { key, note, ...dump };
  const existingIdx = screens.findIndex((s) => s.key === key);
  if (existingIdx >= 0) screens[existingIdx] = screen; else screens.push(screen);
  await persist();
  log(`SCREEN "${key}" url=${dump.url} page=${dump.pageTag} interactive=${dump.interactive.length}${dump._err ? ' ERR=' + dump._err : ''}`);
  for (const it of dump.interactive) {
    const meta = [it.tab ? `tab=${it.tab}` : '', it.href ? `href=${it.href}` : '', it.icons?.length ? `icons=${it.icons.join(',')}` : '', it.labels?.length ? `labels=${it.labels.join('/')}` : ''].filter(Boolean).join(' ');
    log(`   • ${it.kind}${it.inMenu ? '[menu]' : ''}${it.disabled ? '[disabled]' : ''} sel=${it.selector} text="${it.text}" ${meta}`);
  }
  return screen;
}
...
async function clickSel(driver: Driver, selector: string): Promise<boolean> {
  await switchWv(driver);
  const ok = await driver.execute((sel: string) => {
    const vis = (el: Element): boolean => {
      const h = el as HTMLElement; const r = h.getBoundingClientRect(); const s = getComputedStyle(h);
      return s.display !== 'none' && s.visibility !== 'hidden' && r.width > 0 && r.height > 0;
    };
...
async function clickText(driver: Driver, text: string): Promise<boolean> {
  await switchWv(driver);
  const ok = await driver.execute((target: string) => {
    ...
    const t = norm(target);
    const cands = Array.from(document.querySelectorAll('button, [role="button"], ion-button, ion-item, ion-label, ion-tab-button, a, ion-menu-toggle')) as HTMLElement[];
    const clickable = (el: HTMLElement): HTMLElement => {
      let c: HTMLElement | null = el;
      while (c) {
        const tg = c.tagName.toUpperCase();
        if (['BUTTON', 'ION-BUTTON', 'ION-ITEM', 'ION-TAB-BUTTON', 'ION-MENU-TOGGLE', 'A'].includes(tg) || c.getAttribute('role') === 'button') return c;
        c = c.parentElement;
      }
      return el;
    };
    const m = cands.find((el) => vis(el) && [norm(el.innerText || el.textContent), norm(el.getAttribute('aria-label'))].some((v) => v.includes(t)));
    if (m) { clickable(m).click(); return true; }
    return false;
  }, text).catch(() => false);
  await driver.pause(2200);
  return ok as boolean;
...
async function goHome(driver: Driver, timeout = 20_000): Promise<boolean> {
  ...
  while (Date.now() < deadline) {
    ...
    if (await clickText(driver, 'Home')) { /* try */ }
...
async function dismissAlerts(driver: Driver, maxRounds = 4): Promise<string[]> {
  ...
  for (let i = 0; i < maxRounds; i++) {
    ...
    const info = await driver.execute(() => {
      ...
      const msg = norm((modal.querySelector('.alert-message, .message, p, ion-text') as HTMLElement)?.innerText || modal.textContent).slice(0, 120);
      const btn = (Array.from(modal.querySelectorAll('button, [role="button"], ion-button')) as HTMLElement[])
        .find((b) => vis(b) && /aceptar|ok|entendido|cerrar/i.test(norm(b.innerText || b.textContent)));
      if (btn) { btn.click(); return { present: true, msg, clicked: true }; }
      return { present: true, msg, clicked: false };
    }).catch(() => ({ present: false, msg: '', clicked: false })) as { present: boolean; msg: string; clicked: boolean };
    if (!info.present) break;
    if (info.clicked) { dismissed.push(info.msg); log(`Dismissed alert: "${info.msg}"`); await driver.pause(2000); }
    else { log(`Alert present but no Aceptar button: "${info.msg}"`); break; }
  }
  return dismissed;
}
  ...
  while (Date.now() < deadline) {
    ...
    if (/pre-home/i.test(url)) {
      ...
      const clicked = await driver.execute(() => {
        ...
        const btns = Array.from(document.querySelectorAll('button, [role="button"], ion-button, .carrier-overlay')) as HTMLElement[];
        const cont = btns.find((b) => /aceptar|continuar|continue/i.test(norm(b.innerText || b.textContent)) && (b as HTMLElement).offsetParent !== null);
        if (cont) { cont.click(); return 'aceptar'; }
...
async function loginIfNeeded(driver: Driver): Promise<void> {
  await switchWv(driver);
  const url = await getUrl(driver);
  const onLoginNative = await driver.$('//android.widget.EditText[1]').isDisplayed().catch(() => false);
  if (/login|signin/i.test(url) || onLoginNative) {
    ...
    await driver.$('//*[@text="Entrar"]').click().catch(() => {});
...
async function run(): Promise<void> {
  ...
  try {
    ...
    if (cleared.length) transitions.push({ from: 'stale-payment/alert', action: 'tap Aceptar (trip-lost alert)', selector: 'app-alert-modal button.btn-outlined-red', to: 'home', toUrl: cleared.join(' | ') });
    await goHome(driver);
    ...
    const availItem = home.interactive.find((i) => i.id === 'availability' || /disponible|available/i.test(i.text + i.aria));
    ...
    if (availItem && process.env.EXPLORE_SKIP_AVAIL !== '1') {
      ...
      if (clicked) {
        ...
        transitions.push({ from: 'home:initial', action: 'tap #availability', selector: '#availability', to: after.pageTag || 'home', toUrl: after.url });
    ...
    const openMenu = async (): Promise<boolean> => {
      ...
      const shown = await driver.execute(() => {
    ...
    if (menuOpened) {
      ...
      transitions.push({ from: 'home', action: 'open side menu', selector: 'ion-menu-button|Cuenta', to: 'side-menu', toUrl: menu.url });
      ...
      for (const label of menuLabels) {
        ...
        if (!reopened) { log('could not reopen menu'); break; }
        const navigated = await clickText(driver, label);
        if (!navigated) { log(`menu item "${label}" not clickable`); continue; }
        await driver.pause(2500);
        const key = `menu:${label.replace(/\s+/g, '-').toLowerCase()}`;
        const scr = await enumerate(driver, key, `Desde menú lateral → "${label}"`);
        transitions.push({ from: 'side-menu', action: `tap "${label}"`, selector: `text:${label}`, to: scr.pageTag || key, toUrl: scr.url });
      }
      await goHome(driver);
    } else {
    ...
    for (const tab of tabs) {
      ...
      if (label && BLACKLIST.test(label)) { log(`skip destructive tab "${label}"`); continue; }
      // Call/dialer tab is semi-destructive — record but do not click.
      if (/llamar|call/i.test(label)) { log(`recording (not clicking) call tab "${label}" sel=${tab.selector}`); continue; }
      await goHome(driver);
      ...
      transitions.push({ from: 'home', action: `tap tab "${label}"`, selector: tab.selector, to: scr.pageTag || key, toUrl: scr.url });
    }
    await goHome(driver);
...
run().catch((e) => { console.error('[explore] fatal:', e); process.exit(1); });
