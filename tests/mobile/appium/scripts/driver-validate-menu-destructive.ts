...
if (!EMAIL || !PASSWORD) {
	console.error('[menu-destr] ❌ Definir DRIVER_EMAIL y DRIVER_PASSWORD como variables de entorno.');
	process.exit(1);
}
...
async function waitForUrl(d: Driver, token: string, timeout = 20_000): Promise<boolean> {
	...
	while (Date.now() < deadline) {
		await switchWv(d);
		if ((await url(d)).includes(token)) return true;
		await d.pause(500);
	}
	return false;
}
	...
	return d
		.execute<string, []>(() => {
			...
			const h = document.querySelector(
				'page-home ion-header, page-home .header-info, ion-header'
			) as HTMLElement | null;
...
async function openMenu(d: Driver): Promise<boolean> {
	...
	const opened = await d
		.execute<boolean, []>(() => {
			...
			const tog = Array.from(document.querySelectorAll('ion-menu-toggle')).find(
				t => (t as HTMLElement).offsetParent !== null
			) as HTMLElement | null;
			if (tog) {
				tog.click();
				return true;
...
async function dumpMenu(d: Driver): Promise<Record<string, unknown>> {
	...
	return d
		.execute<Record<string, unknown>, []>(() => {
			...
			const items = Array.from(document.querySelectorAll('ion-item.menu-link-url, ion-menu ion-item'))
				.filter(vis)
				.map(it => norm((it as HTMLElement).innerText).slice(0, 40));
			const toggles = Array.from(document.querySelectorAll('ion-menu ion-toggle, ion-toggle'))
				.filter(vis)
				.map(t => ({
					checked:
						t.getAttribute('aria-checked') ??
						String((t as HTMLElement).classList.contains('toggle-checked')),
					text: norm(t.closest('ion-item')?.textContent ?? '').slice(0, 40)
				}));
			...
			(em: string, pw: string) => {
				const set = (host: Element | null, val: string): boolean => {
					...
					const inner = (host.querySelector('input') ||
						(host as HTMLElement & { shadowRoot?: ShadowRoot }).shadowRoot?.querySelector(
							'input'
				...
				for (const ion of ions) {
					const inner =
						...
						(ion as HTMLElement & { shadowRoot?: ShadowRoot }).shadowRoot?.querySelector('input');
					const type = inner?.getAttribute('type') || ion.getAttribute('type') || '';
					if (/password/i.test(type)) passHost = ion;
					else if (!emailHost) emailHost = ion;
				}
				const okE = set(emailHost, em);
				const okP = set(passHost, pw);
				const btn = (
					Array.from(document.querySelectorAll('button.btn.primary, button, ion-button')) as HTMLElement[]
				).find(
					b =>
						/entrar|ingresar|login|iniciar/i.test(b.innerText || b.textContent || '') &&
						b.offsetParent !== null
				);
				if (btn) {
					btn.click();
					return `email=${okE} pass=${okP} → Entrar`;
				}
	...
	while (Date.now() < deadline) {
		...
		if (/pre-home/i.test(u)) {
			await d
				.execute<void, []>(() => {
					const b = (
						Array.from(document.querySelectorAll('button.btn.primary, button')) as HTMLElement[]
					).find(
						x => /aceptar|continuar/i.test(x.innerText || x.textContent || '') && x.offsetParent !== null
					);
					if (b) b.click();
				})
				.catch(() => {});
...
async function run(): Promise<void> {
	...
	try {
		...
		try {
			...
			const settingsDump = await driver
				.execute<Record<string, unknown>, []>(() => {
					const norm = (v: unknown): string =>
						String(v ?? '')
							.replace(/\s+/g, ' ')
							.trim();
					const vis = (el: Element): boolean => (el as HTMLElement).offsetParent !== null;
					const selects = Array.from(document.querySelectorAll('ion-select'))
						.filter(vis)
						.map(s => norm((s as HTMLElement).innerText).slice(0, 40));
					const toggles = Array.from(document.querySelectorAll('ion-toggle'))
						.filter(vis)
						.map(t => t.getAttribute('aria-checked') ?? '');
					const saveBtn = Array.from(document.querySelectorAll('button.btn.primary, button')).find(
						b => /guardar/i.test((b as HTMLElement).innerText) && vis(b)
					);
			...
			const saved = await driver
				.execute<boolean, []>(() => {
					const b = Array.from(document.querySelectorAll('button.btn.primary, button')).find(
						x => /guardar/i.test((x as HTMLElement).innerText) && (x as HTMLElement).offsetParent !== null
					) as HTMLElement | null;
					if (b) {
						b.click();
						return true;
			...
			add(
				'2d-A-Settings',
				okItem && atSettings ? 'OK' : 'PARTIAL',
				`atSettings=${atSettings} guardar=${saved} → ${afterSave} | dump=${JSON.stringify(settingsDump)}`
			);
			await backHome(driver);
		} catch (e) {
			add('2d-A-Settings', 'FAIL', e instanceof Error ? e.message : String(e));
			await backHome(driver).catch(() => {});
		}
		...
		try {
				...
				.execute<Record<string, unknown>, []>(() => {
					const norm = (v: unknown): string =>
						String(v ?? '')
							...
							.trim();
					const vis = (el: Element): boolean => (el as HTMLElement).offsetParent !== null;
					const rows = Array.from(document.querySelectorAll('ion-item, .vehicle-item, ion-card, ion-radio'))
						.filter(vis)
						.map(r => norm((r as HTMLElement).innerText).slice(0, 60))
						.filter(t => t.length > 1)
						.slice(0, 12);
			...
			add(
				'2d-B-Vehicles',
				okItem && atVehicles ? 'OK' : 'PARTIAL',
				`atVehicles=${atVehicles} | ${JSON.stringify(vehDump)} | (no se seleccionó otro vehículo para no cambiar el activo)`
			);
			await backHome(driver);
			const vehAfter = await headerVehicle(driver);
			add(
				'2d-B-Vehicles-verify',
				vehAfter === vehBefore ? 'OK' : 'PARTIAL',
				`vehículo header before="${vehBefore}" after="${vehAfter}"`
			);
		} catch (e) {
			add('2d-B-Vehicles', 'FAIL', e instanceof Error ? e.message : String(e));
			await backHome(driver).catch(() => {});
		}
		...
		try {
			...
			const before = await driver
				.execute<string, []>(() => {
					...
					return t
						? (t.getAttribute('aria-checked') ?? String(t.classList.contains('toggle-checked')))
						: '(none)';
				})
				.catch(() => '(none)');
			...
			await driver
				.execute<void, []>(() => {
					const b = document.querySelector(
						'app-confirm-modal button.btn.primary, ion-alert button'
					) as HTMLElement | null;
			...
			add(
				'2d-C-OutOfService',
				tapped ? 'OK' : 'PARTIAL',
				`toggle checked before="${before}" mid="${mid}" afterRevert="${after}" (reverted=${before === after})`
			);
			await backHome(driver);
		} catch (e) {
			add('2d-C-OutOfService', 'FAIL', e instanceof Error ? e.message : String(e));
			await backHome(driver).catch(() => {});
		}
		...
		try {
			await openMenu(driver);
			const tappedLogout = await driver
				.execute<boolean, []>(() => {
			...
			add(
				'2d-D-Logout',
				tappedLogout && atLogin ? 'OK' : 'FAIL',
				`tapped=${tappedLogout} atLogin=${atLogin} url=${await url(driver)}`
			);
		} catch (e) {
			add('2d-D-Logout', 'FAIL', e instanceof Error ? e.message : String(e));
		}
		...
		try {
			...
			if (/\/login/i.test(await url(driver))) {
				const tappedRestore = await driver
					.execute<boolean, []>(() => {
					...
					.catch(() => false);
				await driver.pause(2000);
				const restoreDump = await driver
					.execute<Record<string, unknown>, []>(() => {
						const norm = (v: unknown): string =>
							String(v ?? '')
								.replace(/\s+/g, ' ')
								.trim();
						const vis = (el: Element): boolean => (el as HTMLElement).offsetParent !== null;
						const inputs = Array.from(document.querySelectorAll('ion-input, input'))
							.filter(vis)
							.map(i =>
								norm(
									(i as HTMLElement).getAttribute('placeholder') ?? (i as HTMLElement).innerText
								).slice(0, 40)
							);
						const title = norm(document.querySelector('ion-title, h1, h2, .title')?.textContent ?? '');
						const btns = Array.from(document.querySelectorAll('button, ion-button'))
							.filter(vis)
							.map(b => norm((b as HTMLElement).innerText).slice(0, 30))
							.filter(t => t.length > 0);
				...
				add(
					'2d-E-RestorePass',
					tappedRestore ? 'OK' : 'PARTIAL',
					`tapped=${tappedRestore} → ${JSON.stringify(restoreDump)}`
				);
				// volver a /login
				await driver
					.execute<void, []>(() => {
				...
				add(
					'2d-E-RestorePass-back',
					/\/login/i.test(await url(driver)) ? 'OK' : 'PARTIAL',
					`volvió a ${await url(driver)}`
				);
			} else {
				add('2d-E-RestorePass', 'PARTIAL', `no en /login (${await url(driver)}) — se omite RestorePass`);
			}
		} catch (e) {
			add('2d-E-RestorePass', 'FAIL', e instanceof Error ? e.message : String(e));
		}
		...
		try {
			...
			add(
				'2d-F-Disponible',
				/disponible/i.test(avail) && !/no disponible/i.test(avail) ? 'OK' : 'PARTIAL',
				`availability="${avail}" url=${await url(driver)}`
			);
		} catch (e) {
			add('2d-F-Relogin', 'FAIL', e instanceof Error ? e.message : String(e));
		}
	} catch (e) {
		...
		const ts = new Date().toISOString().replace(/[:.]/g, '-');
		writeFileSync(
			join('evidence/reports', `menu-destructive-${ts}.json`),
...
run().catch(e => {
	console.error('[menu-destr] fatal:', e);
	process.exit(1);
});
