...
async function readHomeControls(driver: Driver): Promise<Record<string, unknown>> {
  ...
  return driver.execute<Record<string, unknown>, []>(() => {
    ...
    const passLabel = document.querySelector('div.driver-pass.home-icon .pass-label, .pass-label') as HTMLElement | null;
    return {
      url: window.location.href,
      availabilityFound: !!avail,
      availabilityText: norm(avail?.querySelector('.available-label')?.textContent ?? avail?.textContent ?? ''),
      baseFound: !!base,
...
async function dumpOverlays(driver: Driver): Promise<Record<string, unknown>> {
  ...
  return driver.execute<Record<string, unknown>, []>(() => {
    ...
    const sels = ['ion-alert', 'app-confirm-modal', 'ion-modal', 'app-code-confirmation-modal', 'app-alert-modal'];
    for (const sel of sels) {
      document.querySelectorAll(sel).forEach((el) => {
        ...
        const buttons = Array.from(el.querySelectorAll('button, .alert-button, [role="button"]'))
          .filter((b) => vis(b))
          .map((b) => ({ text: norm((b as HTMLElement).innerText ?? b.textContent), class: norm((b as HTMLElement).className).slice(0, 80) }));
...
async function tapPasajero(driver: Driver): Promise<boolean> {
  ...
  return driver.execute<boolean, []>(() => {
    ...
    if (img && img.offsetParent !== null) { img.click(); return true; }
    const div = active.querySelector('div.driver-pass.home-icon') as HTMLElement | null;
    if (div && div.offsetParent !== null) { div.click(); return true; }
    return false;
  }).catch(() => false);
}
...
async function tapConfirmPrimary(driver: Driver): Promise<string> {
  ...
  return driver.execute<string, []>(() => {
    ...
    for (const m of cms) {
      ...
      if (b && vis(b)) { b.click(); return `app-confirm-modal:${norm(b.innerText)}`; }
    }
    ...
    for (const a of alerts) {
      ...
      const target = btns.find((b) => vis(b) && /^(aceptar|s[ií]|confirmar|ok)$/i.test(norm(b.innerText || b.textContent)));
      if (target) { target.click(); return `ion-alert:${norm(target.innerText || target.textContent)}`; }
    }
    return '';
  }).catch(() => '');
}
...
async function tapConfirmCancel(driver: Driver): Promise<string> {
  ...
  return driver.execute<string, []>(() => {
    ...
    const cms = Array.from(document.querySelectorAll('app-confirm-modal')).filter(vis);
    for (const m of cms) {
      const b = (m.querySelector('button.btn-outlined-red') as HTMLElement | null)
        ?? (Array.from(m.querySelectorAll('button')) as HTMLElement[]).find((x) => vis(x) && /^(no|cancelar)$/i.test(norm(x.innerText)));
      if (b && vis(b)) { b.click(); return `app-confirm-modal:${norm(b.innerText)}`; }
    }
    const alerts = Array.from(document.querySelectorAll('ion-alert')).filter(vis);
    for (const a of alerts) {
      const btns = Array.from(a.querySelectorAll('button, .alert-button')) as HTMLElement[];
      const target = btns.find((b) => vis(b) && /^(no|cancelar|cancel)$/i.test(norm(b.innerText || b.textContent)));
      if (target) { target.click(); return `ion-alert:${norm(target.innerText || target.textContent)}`; }
    }
    return '';
  }).catch(() => '');
...
async function cleanupStreetTrip(driver: Driver): Promise<string> {
  ...
  if (/TravelToStartPage/i.test(u)) {
    await driver.execute<boolean, []>(() => {
      const b = document.querySelector('app-page-travel-to-start:not(.ion-page-hidden) ion-footer button, button.btn.primary.trip-pax-start') as HTMLElement | null;
  ...
  if (/TravelInProgressPage/i.test(u)) {
    await driver.execute<boolean, []>(() => {
      ...
      const b = (active?.querySelector('.btn-finish-container button') as HTMLElement | null)
  ...
  if (/TravelResumePage/i.test(u)) {
    for (let i = 0; i < 6; i++) {
      ...
      if (!/ingresar tarjeta/i.test(fs.text) && !fs.disabled && fs.text.length > 0) {
        ...
        await driver.execute<boolean, []>(() => {
          const s = document.querySelector('app-page-signer'); return !!s;
        }).catch(() => false);
        await tapConfirmPrimary(driver);
        break;
      }
...
async function run(): Promise<void> {
  ...
  try {
    ...
    if (!/\/navigator\/home/i.test(u0)) {
      add('PRECOND', 'FAIL', `no en /navigator/home (${u0}). Correr driver-relogin-and-home / driver-go-online primero.`);
      return;
    }

    const baseline = await readHomeControls(driver);
    save('00-baseline', JSON.stringify(baseline, null, 2));
    add('PRECOND', 'OK', `home. availability="${baseline.availabilityText}" base="${baseline.baseText}" pass="${baseline.passLabel}" (baseFound=${baseline.baseFound} passFound=${baseline.passFound})`);
    ...
    if (!tapped0) {
      add('TASK-0', 'FAIL', 'no se pudo tapear div.driver-pass.home-icon (botón Pasajero) — selector no visible.');
    } else if (overlayCount > 0 && !startedTrip) {
      const looksStreet = /viaje|calle|pasajero|iniciar|empezar|street/i.test(overlayText);
      add('TASK-0', 'OK', `tap Pasajero abrió diálogo de confirmación (overlays=${overlayCount}, street-msg=${looksStreet}). En Base sin cambio: "${baseBefore}"→"${baseAfter}". NO togglea En Base ✓`);
    } else if (startedTrip) {
      add('TASK-0', 'PARTIAL', `tap Pasajero navegó directo a ${afterTap.url} (sin diálogo intermedio detectado). Igual confirma que NO es toggle En Base.`);
    } else {
      add('TASK-0', 'FAIL', `tap Pasajero no abrió diálogo ni navegó (overlays=${overlayCount}). base "${baseBefore}"→"${baseAfter}".`);
    }
    ...
    if (!startedTrip) {
      add('TASK-0-dismiss', /Cancelar|No/i.test(cancelled) || cancelled ? 'OK' : 'PARTIAL', `dismiss="${cancelled}" → url=${postCancel.url}`);
    }
    ...
    if (!baseState0.baseFound) {
      add('TASK-2b', 'PARTIAL', 'button.driver-home.home-icon-base no presente en este home (sub-estado base no renderizado sin geocerca de base).');
    } else {
      ...
      let confirmB = '';
      if (Number(overlaysB.overlayCount ?? 0) > 0) { confirmB = await tapConfirmPrimary(driver); await driver.pause(1500); }
      const baseState1 = await readHomeControls(driver);
      const label1 = String(baseState1.baseText ?? '');
      add('TASK-2b', tappedB ? 'OK' : 'FAIL',
      ...
      if (/\/navigator\/home/i.test(u1)) {
        ...
        if (Number(ovR.overlayCount ?? 0) > 0) { await tapConfirmPrimary(driver); await driver.pause(1200); }
        const baseState2 = await readHomeControls(driver);
        add('TASK-2b-revert', 'INFO', `revert label "${label1}"→"${baseState2.baseText}" url=${baseState2.url}`);
      } else {
        add('TASK-2b-revert', 'PARTIAL', `tras toggle no estamos en home (${u1}); revert no ejecutado por seguridad.`);
      }
    }
    ...
    if (!/\/navigator\/home/i.test(preC)) {
      ...
      for (const tok of ['TravelInProgressPage', 'TravelToStartPage', 'TravelConfirmPage']) {
        if (await waitForUrl(driver, tok, 8_000)) { reached = tok; break; }
      }
      const afterC = await url(driver);
      save('task2c-after', JSON.stringify({ tappedC, confirmC, reached, afterC, overlaysC }, null, 2));
      if (reached) {
        add('TASK-2c', 'OK', `street-trip arrancó: tap=${tappedC} confirm="${confirmC}" → ${reached} (${afterC})`);
      } else if (confirmC) {
        add('TASK-2c', 'PARTIAL', `confirm="${confirmC}" pero no se detectó Travel*Page (url=${afterC}).`);
      } else {
        add('TASK-2c', 'FAIL', `no confirm/no transición. tap=${tappedC} overlays=${overlaysC.overlayCount} url=${afterC}`);
      }
      // cleanup
      log('── cleanup street-trip ──');
      const finalUrl = await cleanupStreetTrip(driver);
      add('TASK-2c-cleanup', /\/navigator\/home/i.test(finalUrl) ? 'OK' : 'FAIL', `url final tras cleanup: ${finalUrl}`);
    }
    ...
    add('FINAL', /\/navigator\/home/i.test(String(finalState.url ?? '')) ? 'OK' : 'PARTIAL',
    ...
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    writeFileSync(join('evidence/reports', `home-street-base-${ts}.json`), JSON.stringify({ timestamp: new Date().toISOString(), report }, null, 2), 'utf-8');
    await driver.deleteSession().catch(() => {});
    log('Sesión cerrada');
  }
}
run().catch((e) => { console.error('[home-street-base] fatal:', e); process.exit(1); });
