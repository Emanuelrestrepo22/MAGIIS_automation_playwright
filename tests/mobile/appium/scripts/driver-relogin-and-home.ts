	...
	return driver
		.execute(
			(em: string, pw: string) => {
				...
				// Fallback: raw inputs
				if (!emailHost)
					emailHost = document.querySelector(
						'input[type="email"], input[type="text"], input:not([type="password"])'
					);
				if (!passHost)
					passHost =
						document.querySelector('ion-input[type="password"]') ||
						document.querySelector('input[type="password"]')?.closest('ion-input') ||
						null;
				...
				const btn = (
					Array.from(
						document.querySelectorAll('button.btn.primary, button, [role="button"], ion-button')
					) as HTMLElement[]
				).find(
					b =>
						/entrar|ingresar|login|iniciar/i.test(norm(b.innerText || b.textContent)) &&
						b.offsetParent !== null
				);
				if (btn) {
					btn.click();
					return `filled email=${okE} pass=${okP} -> Entrar clicked`;
				}
			...
			const tapped = await driver
				.execute<string, []>(() => {
					...
					const b = (
						...
						x => /aceptar|continuar/i.test(norm(x.innerText || x.textContent)) && x.offsetParent !== null
					);
					if (b) {
						b.click();
						return 'tapped';
...
async function run(): Promise<void> {
	...
	try {
		...
		const hasAvail = await driver
			.execute<boolean, []>(() => !!document.querySelector('#availability'))
			.catch(() => false);
