...
async function run(): Promise<void> {
	...
	).catch((e: unknown) => { log(`err join: ${e instanceof Error ? e.message : String(e)}`); return []; });
	...
	const prog = await db.query<Record<string, unknown>>(
	...
	).catch((e: unknown) => { log(`err prog: ${e instanceof Error ? e.message : String(e)}`); return []; });
	...
	const leak = await db.query<Record<string, unknown>>(
		`SELECT h.ID AS "HOLD", h.TRAVEL_ID AS "TRAVEL", h.STATUS AS "HOLD_ST", h.AMOUNT_HOLD AS "AMT",
...
run().catch((e: unknown) => { console.error(`[sched-hold] ${e instanceof Error ? e.message : String(e)}`); process.exit(1); });
