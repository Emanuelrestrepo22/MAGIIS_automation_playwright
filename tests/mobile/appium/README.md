...
- Wallet principal action: tap `button.card-item-opts` on the target card and then select `Principal` from the popover
- Wallet delete action: the same popover exposes `Eliminar`
...
- The canonical end-to-end personal 3DS + hold runner is `pnpm mobile:passenger:personal-3ds-hold-flow`.
