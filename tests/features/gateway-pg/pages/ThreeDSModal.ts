...
/**
 * Stripe 3DS Challenge Modal — POM propio de la feature gateway-pg.
 *
 * Modela el modal de autenticación 3DS que aparece sobre el iframe Stripe
 * Elements. Authorize.net NO usa 3DS → este POM NO aplica a otros gateways.
 *
 * Reubicado a `features/gateway-pg/pages/` en 2026-07-14 (encapsulación Fase B).
 * Las rutas viejas (`pages/carrier/stripe/ThreeDSModal`, `pages/carrier/ThreeDSModal`)
 * quedan como thin re-export shims por compatibilidad.
 */
...
export class ThreeDSModal {
	...
	async waitForHidden(timeout = 45_000): Promise<void> {
		...
		const vehicleButton = this.page
			...
			.filter({ hasText: /Seleccionar Veh[Ã­i]culo/i })
			.first();
