...
test.describe('[MG · F][DB] ePayment — doble cobro / idempotencia @regression @gateway @idempotency', {
	...
	test('[F-02] cobrar + re-cobrar el MISMO viaje → una sola fila APROBADA (idempotencia)', async ({ request }) => {
		const cfg = ORACLE!;
		...
		await epayment.finalizeEpayment({ ePaymentId: ePaymentId1 as number | string, status: 'approved', authToken });
		...
		if (ePaymentId2) {
			await epayment.finalizeEpayment({ ePaymentId: ePaymentId2 as number | string, status: 'approved', authToken });
		}
		...
		await expect
			.poll(async () => countMgwTransactionsByRef(cfg, { transactionRef: TX_REF, statuses: ['APPROVED', 'CONFIRM'] }), {
				message:
