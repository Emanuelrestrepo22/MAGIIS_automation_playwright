...
test.describe(
	`[MG · A][API] vendor/mercadopago — vinculación ${mercadoPagoGatewayAdapter.displayName} @regression @gateway @gateway-pg @mercadopago`,
	{
	...
	() => {
		...
		test(
			'[A-01] vincular MercadoPago con code OAuth → 200/201',
			{
				annotation: [{ type: 'tms', description: 'MG-141' }]
			},
			async ({ request }) => {
				test.skip(
					!CARRIER_USER_ID || !MP_OAUTH_CODE,
					'Faltan MP_CARRIER_USER_ID / MP_OAUTH_CODE (datos de UAT) [confirmar].'
				);
		...
		test(
			'[A-04] re-vincular carrier con MP ya vinculado → 409 MERCADOPAGO_IN_USE',
			{
			...
			async ({ request }) => {
				...
				expect(res.raw, 'el body debe referenciar el código de negocio MERCADOPAGO_IN_USE').toContain(
					'MERCADOPAGO_IN_USE'
				);
			}
		);
