...
test.describe(`[MG · E/F/COB][API] ${mercadoPagoGatewayAdapter.displayName} — ePayment → finalize @regression @gateway @gateway-pg @mercadopago`, {
	...
	test('[E-01] startEpayment con hold → 2077 HOLD_NOT_SUPPORTED', {
		...
		expect(
			res.status === 2077 || String(code) === String(MP_HOLD_NOT_SUPPORTED_CODE) || res.raw.includes('HOLD_NOT_SUPPORTED'),
	...
	test('[COB-01] cobro con tarjeta de rechazo → rejected', {
		annotation: [{ type: 'tms', description: 'MG-163' }]
	}, async ({ request }) => {
		test.skip(!CARRIER_ACCOUNT_ID || !PASSENGER_ID || !MP_REJECT_CARD_ID, 'Faltan CARRIER_ID / MP_PASSENGER_ID / MP_REJECT_CARD_ID (tarjeta OTHE sembrada en UAT) [confirmar].');
		const res = await new EpaymentApi({ request }).startEpayment({
			carrierId: CARRIER_ACCOUNT_ID,
			passengerId: PASSENGER_ID,
