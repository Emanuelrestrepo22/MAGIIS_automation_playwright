...
export function defineCardOutcomeMatrixSuite(gateway: GatewayName, options: CardOutcomeMatrixSuiteOptions = {}): void {
	...
	test.describe(`Gateway PG · Carrier · ${adapter.displayName} — matriz de outcomes de alta de tarjeta @gateway ${gatewayTag(gateway)} @cardmatrix @regression`, () => {
		...
		for (const intent of intents) {
			...
			const etiquetaBase = hasObservedOutcome(intent)
				? outcomeFor(intent).label
				: intent.toLowerCase().replace(/_/g, ' ');
			const etiqueta = areaF
				? `alta APROBADA — ${etiquetaBase} se decide en el área ${areaF.area} (cobro)`
				: etiquetaBase;
				...
				if (addCard.shouldSucceed) {
					...
					await test.step(`Then: el sistema da la tarjeta por válida (${etiquetaThen})`, async () => {
						...
						await travel.validateNativeCard(
							card.last4,
							support.slowMs ? 20_000 + support.slowMs : undefined
						);
					});
				} else {
					...
					await test.step(`Then: el sistema NO da la tarjeta por válida (${expected.label}, base: ${expected.basis})`, async () => {
						try {
							...
							await test
								.info()
								.attach(`trifuerza-decline-${intent}.txt`, {
									body: detalle,
