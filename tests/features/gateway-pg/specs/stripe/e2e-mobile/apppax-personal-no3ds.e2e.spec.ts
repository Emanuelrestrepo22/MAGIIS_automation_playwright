...
test.describe.serial('Gateway PG · E2E Mobile · App Pax Personal No 3DS @gateway @stripe @e2e-hybrid @wallet @regression', () => {
	for (const scenario of PASSENGER_PERSONAL_NO3DS_SCENARIOS) {
		test(
			...
			async () => {
				...
				try {
					...
					switch (scenario.step) {
						case 'wallet-add-card':
							await test.step(`[${scenario.testCaseId}] add card to wallet`, async () => {
								...
								journey = orchestrator.updatePhase(
									...
									'draft',
									`Passenger personal wallet card ${walletState}`
								);
						...
						case 'wallet-select-card':
							await test.step(`[${scenario.testCaseId}] select existing wallet card`, async () => {
								...
								journey = orchestrator.updatePhase(
									...
									'passenger_wallet_setup',
									'draft',
									'Passenger personal selected an existing wallet card'
								);
								await orchestrator.persist(journey);
							});
							break;
						...
						case 'trip-create':
							await test.step(`[${scenario.testCaseId}] create passenger trip`, async () => {
								...
								journey = orchestrator.prepareMobileHandoff(
									journey,
									'Passenger created the personal trip and handed it to the driver lane'
								);
								await orchestrator.persist(journey);
							});
							break;
						...
						default:
							test.fixme(true, 'Unhandled passenger personal flow step. Update the scenario mapping first.');
					}
				} catch (error) {
					journey = orchestrator.fail(
						journey,
						error instanceof Error ? error.message : 'Passenger personal flow failed'
					);
					await orchestrator.persist(journey);
					throw error;
				} finally {
