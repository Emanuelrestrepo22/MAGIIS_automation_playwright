...
test.describe(
	'Gateway PG · Carrier · Colaborador — Hold sin 3DS @gateway @stripe @hold @critical @smoke @regression',
	...
	() => {
		test.describe('Hold ON', () => {
			...
			test('[TS-STRIPE-TC1035] @regression @hold @card-new hold+cobro colaborador sin 3DS — Vincular tarjeta nueva', async ({
				...
				await new CarrierHoldSteps({ page }).runHoldScenario(colaboradorScenario('new', CUSTOM_ADDRESS), {
					hold: 'on',
					threeDs: false
				});
			});
			...
			test('[TS-STRIPE-TC1041] @regression @hold @card-existing hold+cobro colaborador sin 3DS — Usar tarjeta vinculada existente', async ({
				...
				await new CarrierHoldSteps({ page }).runHoldScenario(colaboradorScenario('existing', CUSTOM_ADDRESS), {
					hold: 'on',
					threeDs: false
				});
			});
			...
			test('[TS-STRIPE-TC1043] @regression @hold hold+cobro colaborador sin 3DS variante set 2', async ({
				page
			}) => {
				await new CarrierHoldSteps({ page }).runHoldScenario(colaboradorScenario('new'), {
					hold: 'on',
		...
		test.describe('Hold OFF', () => {
			...
			// DEPRECATED: ver TC canónico TS-STRIPE-TC1034 (fase 2 — duplicado sin card-flow diferenciado)
			test('[TS-STRIPE-TC1044] @regression @hold sin hold colaborador sin 3DS variante set 2', async ({
				page
			}) => {
				await new CarrierHoldSteps({ page }).runHoldScenario(colaboradorScenario('new'), {
					hold: 'off',
