 ...
 * mapear al área CLON del idmap (MG-428+, Level UI) — PENDIENTE REASIGNAR.
 */
...
test.describe('Gateway PG · Carrier · Empresa Individuo — Clonación de Viajes Finalizados @gateway @stripe @hold @3ds @regression', () => {
	test.describe('Sin 3DS', () => {
		test('[TS-STRIPE-P2-TC072] @regression @hold clonación finalizado hold+cobro', async () => {
			test.fixme(
				true,
				'DEUDA TÉCNICA MG-178: clonación de finalizados no desarrollada/estable para pasarela preautorizada + no hay finish web-only en esta release; diferido a próxima iteración de cambios en pasarelas de pago.'
			);
		});
