...
test.describe('Gateway PG · Carrier — Reactivación de Viajes Cancelados @gateway @stripe @hold @regression', () => {
	...
	test.describe('Sin 3DS', () => {
		...
		test('[TS-STRIPE-P2-TC061] @regression sin hold reactivación cancelado', async () => {
			test.fixme(true, 'DEUDA TÉCNICA MG-178: reactivación SIN hold no usa tarjeta preautorizada; diferido a próxima iteración de cambios en pasarelas.');
		});

		test('[TS-STRIPE-P2-TC062] @regression @hold reactivación cancelado hold+cobro variante', async () => {
			test.fixme(true, 'DEUDA TÉCNICA MG-178: variante diferida; el flujo base de reactivación preautorizada queda cubierto por TC060 (reusa CarrierReactivationSteps).');
		});

		test('[TS-STRIPE-P2-TC063] @regression sin hold reactivación cancelado variante', async () => {
			test.fixme(true, 'DEUDA TÉCNICA MG-178: reactivación SIN hold; diferido a próxima iteración de cambios en pasarelas.');
		});
	});

	test.describe('Con 3DS', () => {
		test('[TS-STRIPE-P2-TC064] @regression @3ds @hold reactivación cancelado hold+cobro 3DS', async () => {
			test.fixme(true, 'DEUDA TÉCNICA MG-178: reactivación con 3DS diferida a próxima iteración de pasarelas (flujo hold base en TC060).');
		});

		test('[TS-STRIPE-P2-TC065] @regression @3ds sin hold reactivación cancelado 3DS', async () => {
			test.fixme(true, 'DEUDA TÉCNICA MG-178: reactivación SIN hold + 3DS; diferido a próxima iteración de cambios en pasarelas.');
		});
	});
