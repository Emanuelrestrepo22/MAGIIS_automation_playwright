...
test.describe('Gateway PG · Configuración Pasarela Stripe @gateway @stripe @wallet @regression', () => {
	test.describe('Visualización y estado inicial', () => {
		test('[TS-STRIPE-TC1001] @smoke @web-only visualizar pasarela Stripe no vinculada', async () => {
			...
			test.fixme(
				true,
				'DEUDA TÉCNICA MG-178: configuración de pasarela Stripe (OAuth redirect) no lista para validación automatizada en esta release; diferido a próxima iteración de cambios en pasarelas de pago. POM scaffolded: CarrierGlobalIntegrationsPage.'
			);
		});
	...
	test.describe('Vinculación con credenciales', () => {
		test('[TS-STRIPE-TC1002] @smoke @critical vincular Stripe con credenciales válidas', async () => {
			...
			test.fixme(
				true,
				'MG-178 product-mismatch: Stripe usa OAuth redirect, no credenciales. TC requiere rediseño (ver CarrierGlobalIntegrationsPage.startLink).'
			);
		});
