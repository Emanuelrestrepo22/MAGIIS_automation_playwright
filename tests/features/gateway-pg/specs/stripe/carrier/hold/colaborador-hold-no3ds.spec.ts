...
test.describe('Gateway PG · Carrier · Colaborador — Hold sin 3DS', () => {
	...
	test.describe('Hold ON', () => {
		test('[TS-STRIPE-TC1025] @smoke @hold hold+cobro colaborador sin 3DS', async ({ page }) => {
		...
		test('[TS-STRIPE-TC1027] @regression @hold hold+cobro colaborador sin 3DS variante', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1025');
		});

		test('[TS-STRIPE-TC1029] @regression @hold hold+cobro colaborador sin 3DS (set 2)', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1025');
		});

		test('[TS-STRIPE-TC1031] @regression @hold hold+cobro colaborador sin 3DS variante 2', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1025');
		});

		test('[TS-STRIPE-TC1033] @regression @hold hold+cobro colaborador sin 3DS variante 3', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1025');
		});

		test('[TS-STRIPE-TC1035] @regression @hold hold+cobro colaborador sin 3DS variante 4', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1025');
		});
	});

	test.describe('Hold OFF', () => {
		test('[TS-STRIPE-TC1026] @regression sin hold colaborador sin 3DS', async ({ page }) => {
			const dashboard = new DashboardPage(page);
			const preferences = new OperationalPreferencesPage(page);
			const travel = new NewTravelPage(page);
			const management = new TravelManagementPage(page);
		...
		test('[TS-STRIPE-TC1028] @regression sin hold colaborador sin 3DS variante', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1026');
		});

		test('[TS-STRIPE-TC1030] @regression sin hold colaborador sin 3DS (set 2)', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1026');
		});

		test('[TS-STRIPE-TC1032] @regression sin hold colaborador sin 3DS variante 2', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1026');
		});

		test('[TS-STRIPE-TC1034] @regression sin hold colaborador sin 3DS variante 3', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1026');
		});

		test('[TS-STRIPE-TC1036] @regression sin hold colaborador sin 3DS variante 4', async () => {
			test.fixme(true, 'PENDIENTE: depende de TC1026');
		});
	});
