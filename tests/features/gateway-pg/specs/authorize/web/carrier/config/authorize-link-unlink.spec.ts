 ...
 *
 * ⚠️ FRAGILE: los locators del modal `odnService` y el endpoint de red del link/unlink NO están
 * verificados en vivo. Ver comentarios `// FRAGILE` en AppStoreGatewaysPage. Confirmar antes de
...
test.describe('Gateway PG · Carrier · Configuración Pasarela Authorize.net @gateway @authorize @cfg @regression', () => {
	...
	test(
		'[TS-AUTHORIZE-TC1008] @cfg la request de link/unlink retorna 200 + auditoría',
		{
		...
		async ({ page }) => {
			...
			await test.step('When/Then: la request de vinculación retorna status 200', async () => {
