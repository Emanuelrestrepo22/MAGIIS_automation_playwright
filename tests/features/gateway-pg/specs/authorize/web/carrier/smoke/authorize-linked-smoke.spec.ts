...
test.describe('Gateway PG · Carrier · Smoke Authorize vinculada @gateway @authorize @smoke @regression', () => {
	test.describe.configure({ timeout: 120_000 });

	test('[TS-AUTHORIZE-SMOKE-01] Authorize.Net figura vinculada (Unlink) en el App Store — estado persiste en sesión nueva', async ({
		page
	}) => {
		...
		await test.step('Then: la card Authorize.Net muestra estado vinculado (Unlink/Desvincular)', async () => {
			await expect(appStore.cardFor('authorize'), 'la card Authorize.Net debe estar visible').toBeVisible({
				timeout: 20_000
			});
			...
			await expect(async () => {
				expect(
					await appStore.readState('authorize'),
					'Authorize debe figurar vinculada (Unlink/Desvincular)'
				).toBe('linked');
