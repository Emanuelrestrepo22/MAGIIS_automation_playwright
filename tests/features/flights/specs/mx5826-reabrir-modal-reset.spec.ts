...
test.describe(`[FLIGHT][${env.toUpperCase()}] Reabrir modal tras eliminar vuelo → reset — Portal Carrier`, () => {
	...
	// Trazabilidad Allure↔Jira: tms = Xray Test MX-6137, issue = ticket MX-5826 (render como links vía config allure `links`).
	test(
		'@flight @carrier @regression [TS-MX5826-REABRIR-RESET] Eliminar el vuelo y reabrir el modal → campos vacíos + "Aceptar" deshabilitado (no reasocia)',
		{
			annotation: [
				{ type: 'tms', description: 'MX-6137' },
				{ type: 'issue', description: 'MX-5826' }
			]
		},
