...
test.describe('[E2E-FLOW-1] Driver App @regression @stripe @hybrid-e2e', () => {
	...
	test('FLOW1-TC01-driver-app-mobile-handoff', async () => {
		...
		try {
			...
			journey = orchestrator.completeMobilePhase(ensureJourney(), 'Driver completed the trip in the mobile app');
			...
			if (journey) {
				journey = orchestrator.fail(journey, error instanceof Error ? error.message : 'Driver app flow failed');
