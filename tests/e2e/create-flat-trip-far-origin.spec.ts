  ...
  test('create flat trip far origin and assign to device driver', async ({ page }) => {
    ...
    await test.step('Select vehicle + Send Manual + assign to device driver by name', async () => {
      ...
      const candidates = await page.evaluate(() => {
        ...
        const texts: string[] = [];
        document.querySelectorAll('ion-modal, .modal, [class*="assign"], [class*="candidate"], tbody tr, ion-item, .driver-row').forEach((el) => {
      ...
      const clicked = await page.evaluate((match: string) => {
        const norm = (v: unknown): string => String(v ?? '').replace(/\s+/g, ' ').trim();
        const isAssign = (el: Element): boolean => /^(asignar|assign)(\s*\(\d+\))?$/i.test(norm(el.textContent));
        const re = new RegExp(match, 'i');
        // Candidatas: filas/cards que mencionan al driver, con texto acotado (una sola fila).
        const rows = Array.from(document.querySelectorAll('tr, ion-row, ion-item, li, [class*="row"], [class*="card"], [class*="driver"]'))
          .filter((r) => {
            const t = norm((r as HTMLElement).textContent);
            return re.test(t) && /asignar|assign/i.test(t) && t.length < 170 && (r as HTMLElement).offsetParent !== null;
          })
          // preferir la MÁS específica (texto más corto)
          .sort((a, b) => norm((a as HTMLElement).textContent).length - norm((b as HTMLElement).textContent).length);
        for (const row of rows) {
          const a = Array.from(row.querySelectorAll('button, a, [role="button"], ion-button, .btn, span, div'))
            .find((el) => (el as HTMLElement).offsetParent !== null && isAssign(el));
          if (a) { (a as HTMLElement).click(); return `${norm(a.textContent)} | row: ${norm(row.textContent).slice(0, 110)}`; }
        }
        return '';
      }, DRIVER_MATCH);
      console.log(`[web-2a] assign-to-device-driver click: "${clicked}"`);
      expect(clicked, `No se encontró fila "Asignar" para el driver "${DRIVER_MATCH}". Candidatos: ${JSON.stringify(candidates)}`).not.toBe('');
      ...
      const postAssignDialog = await page.evaluate(() => {
        ...
        for (const sel of ['ion-alert', 'app-confirm-modal', '.modal-content', '[class*="modal"]', '[role="dialog"]', '.swal2-popup']) {
          document.querySelectorAll(sel).forEach((el) => {
            if (!vis(el)) return;
            const buttons = Array.from(el.querySelectorAll('button, .alert-button, a, [role="button"]')).filter(vis).map((b) => norm((b as HTMLElement).innerText)).filter((t) => t.length > 0);
      ...
      const confirmed = await page.evaluate(() => {
        ...
        const btns = Array.from(document.querySelectorAll('ion-alert button, .alert-button, app-confirm-modal button, .modal-content button, [role="dialog"] button, button'))
          .filter((b) => vis(b) && RE.test(norm(b.textContent)));
        if (btns.length) { (btns[btns.length - 1] as HTMLElement).click(); return norm(btns[btns.length - 1].textContent); }
        return '';
      });
