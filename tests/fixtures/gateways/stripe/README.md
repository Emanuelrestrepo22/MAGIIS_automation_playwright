...
Source of truth para tarjetas Stripe en todo el proyecto MAGIIS.

> **Ubicación canónica:** `tests/fixtures/gateways/stripe/` (BL-024 Fase 3, 2026-05-13).
> El path legacy `tests/fixtures/stripe/` queda como thin re-export.
...
- Las tarjetas son SoT **fija** en `cards.ts` + `card-policy.ts` (policy por intención). No se generan con faker.
...
```
tests/fixtures/gateways/stripe/
├── cards.ts          ← SoT canónica (StripeTestCard type + STRIPE_TEST_CARDS_RAW + env-aware STRIPE_TEST_CARDS)
├── card-policy.ts    ← namespace CARDS con nombres por INTENCIÓN
├── card-resolver.ts  ← resolveCard(cardId) → StripeTestCard
...
| `DECLINE_INVALID_CVC` | 4000 0000 0000 0127 | 0127 | Decline por CVC incorrecto |
...
- Pre-push check 2 bloquea uso fuera de archivos allowlisted.

## Agregar una card nueva

1. Si el número de tarjeta no existe todavía, agregarla en `cards.ts` (registry `STRIPE_TEST_CARDS_RAW`) con JSDoc del comportamiento Stripe.
2. Si necesita env override en UAT/PROD, agregar entry en `STRIPE_TEST_CARDS` con `resolveCardNumber(...)`.
