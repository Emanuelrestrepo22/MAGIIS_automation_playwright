/**
 * Stripe Card Resolver — Convierte intención al objeto StripeTestCard completo.
 * ================================================================================
 *
 * BL-024 Fase 3 (2026-05-13) — la SoT canónica vive bajo `fixtures/gateways/stripe/`.
 * El archivo `tests/fixtures/stripe/card-resolver.ts` queda como thin re-export.
 *
 ...
 * Uso en specs:
 *
 *   import { resolveCard } from 'tests/fixtures/gateways/stripe/card-resolver';
...
export function resolveCard(cardId: CardId): StripeTestCard {
	...
	if (!found) {
		throw new Error(
			`Card '${cardId}' (number: ${cardNumber}) no existe en STRIPE_TEST_CARD_FIXTURES. Agregarla al registry en tests/fixtures/gateways/stripe/cards.ts.`
		);
	}
	return found;
}
